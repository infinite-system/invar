...
never backlog, never re-proposed — the user raising it is the only exit), `draft`
(spec awaiting user review — logged in-repo, never /tmp-only; exits by blessing into
active or by parking/retiring). Move a task between states. Never delete a folder. The protocol is
...
frame probe), never internal values. Driving goes through the warm fluent server, always (user policy 2026-08-03; `bun run drive` is now an ALIAS of DriveSession.ts — the one-shot Drive.ts is REMOVED). Quick sightings previously used the one-shot
