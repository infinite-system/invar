...
fold ranges preserve snapshot identity across non-structural edits, their collapsed projection uses
zero-default typed headers and span-local block updates, serialized document length is a running
count, and diff overview projection is cached by immutable alignment plus track height. Measured
...
and span-local fold-toggle counts. Millisecond samples remain report-only;
the operation count is the blocking scale claim.
