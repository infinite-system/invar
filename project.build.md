...
```bash
cd /tmp/tui-demo && PATH="$HOME/.bun/bin:$PATH" bun run build && ./dist/iv .
```
...
```bash
bun run build          # → dist/iv  (self-contained executable, ~30–60s)
./dist/iv .         # run the compiled binary against the current dir
```

- `build` = `bun build --compile --minify --external web-tree-sitter src/main.ts --outfile dist/iv`.
...
(regex highlighting), and edits without touching the wasm path — you can run `./dist/iv` from
anywhere.
