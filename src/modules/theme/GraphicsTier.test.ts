...
test("a report beats contradicting env hints in both directions", () => {
  ...
  expect(
    withEnv({ TERM: "xterm-kitty", KITTY_WINDOW_ID: "1" }, reportedNone),
  ).toBe("halfblock");
...
test("tmux forces half-block regardless of reported riches (passthrough is unreliable)", () => {
  expect(withEnv({ TMUX: "/tmp/tmux-1000/default,123,0" }, reportedAll)).toBe(
    "halfblock",
  );
  expect(
    withEnv({}, { kitty_graphics: true, sixel: true, multiplexer: "tmux" }),
  ).toBe("halfblock");
  expect(
    withEnv({}, { kitty_graphics: true, sixel: true, multiplexer: "screen" }),
  ).toBe("halfblock");
  // 'unknown' is the struct default, not a detected multiplexer — it must NOT trip the guard.
  expect(
    withEnv({}, { kitty_graphics: true, sixel: true, multiplexer: "unknown" }),
  ).toBe("kitty");
});
...
test("no report yet: conservative env heuristics, kitty terms first", () => {
  ...
  expect(withEnv({ TERM: "xterm-256color", KITTY_WINDOW_ID: "2" }, null)).toBe(
    "kitty",
  );
  expect(
    withEnv({ TERM: "xterm-256color", TERM_PROGRAM: "WezTerm" }, null),
  ).toBe("sixel");
  expect(
    withEnv({ TERM: "xterm-256color", TERM_PROGRAM: "iTerm.app" }, null),
  ).toBe("sixel");
});
...
test("TUI_GRAPHICS_TIER forces any tier, beating the report AND the tmux guard (the smoke seam)", () => {
  ...
  expect(withEnv({ TUI_GRAPHICS_TIER: "halfblock" }, reportedAll)).toBe(
    "halfblock",
  );
