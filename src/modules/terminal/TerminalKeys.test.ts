...
test('a printable character rides its own sequence', () => {
  expect(TerminalKeys.Class.encode(key({ name: 'a', sequence: 'a' }))).toBe(
    'a',
  );
});
...
test('Ctrl+C encodes the interrupt control byte 0x03', () => {
  expect(
    TerminalKeys.Class.encode(key({ name: 'c', ctrl: true, sequence: '\x03' })),
  ).toBe('\x03');
});

test('Ctrl+D encodes EOF 0x04', () => {
  expect(TerminalKeys.Class.encode(key({ name: 'd', ctrl: true }))).toBe(
    '\x04',
  );
});
...
test('Shift+Tab encodes the back-tab sequence', () => {
  expect(TerminalKeys.Class.encode(key({ name: 'tab', shift: true }))).toBe(
    '\x1b[Z',
  );
});
...
test('word operations forward canonical readline meta sequences', () => {
  expect(TerminalKeys.Class.encode(key({ name: 'left', option: true }))).toBe(
    '\x1bb',
  );
  expect(TerminalKeys.Class.encode(key({ name: 'right', meta: true }))).toBe(
    '\x1bf',
  );
  expect(
    TerminalKeys.Class.encode(key({ name: 'backspace', meta: true })),
  ).toBe('\x1b\x7f');
});
