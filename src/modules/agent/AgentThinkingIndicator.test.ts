...
const baseState = (
  overrides: Partial<
    Parameters<typeof AgentThinkingIndicator.Class.compose>[0]
  > = {},
) => ({
  frameIndex: 0,
  elapsedSeconds: 0,
...
describe('AgentThinkingIndicator word list (user-LOCKED)', () => {
  ...
  test('every pick is a core word OR an easter-egg (never anything else)', () => {
    const known = new Set<string>([
      ...AgentThinkingIndicator.Class.coreWords,
      ...AgentThinkingIndicator.Class.easterEggs,
    ]);
    for (let slot = 0; slot < 2000; slot += 1)
      expect(known.has(AgentThinkingIndicator.Class.pickWord(slot))).toBe(true);
  });
  ...
  test('easter-eggs surface roughly 1-in-15 picks (a rare discovery)', () => {
    ...
    let eggCount = 0;
    const trials = 6000;
    for (let slot = 0; slot < trials; slot += 1)
      if (eggs.has(AgentThinkingIndicator.Class.pickWord(slot))) eggCount += 1;
    const ratio = eggCount / trials;
    const expected = 1 / AgentThinkingIndicator.Class.EASTER_EGG_ODDS;
...
describe('AgentThinkingIndicator.compose', () => {
  test('shows a rotating reduction word (from the locked set) + the elapsed counter', () => {
    const composed = AgentThinkingIndicator.Class.compose(
      baseState({ elapsedSeconds: 5 }),
    );
  ...
  test('the word ROTATES as elapsed time crosses the rotation interval', () => {
    const early = text(
      AgentThinkingIndicator.Class.compose(baseState({ elapsedSeconds: 0 })),
    );
    const later = text(
      AgentThinkingIndicator.Class.compose(baseState({ elapsedSeconds: 9 })),
    );
  ...
  test('EXACTLY ONE leading single-cell glyph, a fixed word column, and NO trailing glyph — no reflow', () => {
    for (const frameIndex of [0, 1, 2, 3, 5, 7, 11, 23]) {
      const composed = AgentThinkingIndicator.Class.compose(
        baseState({ frameIndex, elapsedSeconds: 4 }),
      );
  ...
  test('the per-character shimmer gives the WORD glyphs DIFFERENT colours (truecolor gradient)', () => {
    const composed = AgentThinkingIndicator.Class.compose(
      baseState({ elapsedSeconds: 0, frameIndex: 3 }),
    );
    // Word glyphs = the bold single-cell segments AFTER the leading spinner glyph (segment 0).
    const wordColors = composed
      .filter(
        (segment) => segment.bold && Array.from(segment.text).length === 1,
      )
      .slice(1)
  ...
  test('the ascii tier uses a plain word (single colour) and no braille', () => {
    const composed = AgentThinkingIndicator.Class.compose(
      baseState({ glyphLevel: 'ascii', colorDepth: '16' }),
    );
...
describe('AgentThinkingIndicator.composeWaitingNote', () => {
  ...
  test('nothing pending → no note', () => {
    expect(
      AgentThinkingIndicator.Class.composeWaitingNote(
        noteState({ toolName: null }),
      ),
    ).toEqual([]);
  });
  ...
  test('one pending → the tool name + its elapsed time', () => {
    const joined = text(
      AgentThinkingIndicator.Class.composeWaitingNote(noteState()),
    );
  ...
  test('multiple pending → the active tool plus a pending counter', () => {
    const joined = text(
      AgentThinkingIndicator.Class.composeWaitingNote(
        noteState({ toolName: 'Read', pendingCount: 3 }),
      ),
    );
