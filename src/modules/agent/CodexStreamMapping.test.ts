...
describe('CodexStreamMapping (envelope confirmed via live probe; items best-effort)', () => {
  ...
  test('turn.completed → session-end completed', () => {
    expect(map({ type: 'turn.completed', usage: {} })).toEqual([
  ...
  test('turn.failed → error (with message) + session-end error', () => {
    expect(
      map({ type: 'turn.failed', error: { message: 'usage limit' } }),
    ).toEqual([
  ...
  test('top-level error event → error', () => {
    expect(map({ type: 'error', message: 'boom' })).toEqual([
  ...
  test('item.completed assistant_message → text-delta', () => {
    const event = {
      type: 'item.completed',
      item: { type: 'assistant_message', text: 'hello from codex' },
    };
    expect(map(event)).toEqual([
      { kind: 'text-delta', text: 'hello from codex' },
    ]);
  });
