...
describe('mapClaudeStreamEvent', () => {
  ...
  test('assistant text block → text-delta with the exact text', () => {
    expect(mapClaudeStreamEvent(ASSISTANT_TEXT)).toEqual([
      { kind: 'text-delta', text: 'hello world' },
    ]);
  });
  ...
  test('tool_result with array content joins the text parts', () => {
    ...
    expect(mapClaudeStreamEvent(event)).toEqual([
      { kind: 'tool-result', id: 't', result: 'ab', isError: true },
    ]);
  });

  test('result → session-end, error flag honored', () => {
    expect(mapClaudeStreamEvent(RESULT_OK)).toEqual([
      { kind: 'session-end', reason: 'completed' },
    ]);
    expect(mapClaudeStreamEvent(RESULT_ERR)).toEqual([
