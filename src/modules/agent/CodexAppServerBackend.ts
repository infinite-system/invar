...
class $CodexAppServerBackend implements AgentBackend {
  ...
  protected async ensureThread(bypassPermissions: boolean): Promise<void> {
    ...
    if (!this.threadId) {
      const started = (await this.request('thread/start', {
        ...
        ...(this.options.cwd
          ? { cwd: Files.Class.absolute(this.options.cwd) }
          : {}),
  ...
  protected write(payload: unknown): void {
    const sink = this.child?.stdin as unknown as {
  ...
  protected consumeLine(line: string): void {
    ...
    if (id !== undefined && typeof message.method === 'string') {
      ...
      const approval = CodexAppServerMapping.Class.approvalOf(
        message.method,
        message.params,
      );
      if (approval) this.emitPermissionRequest(id as number | string, approval);
        ...
        this.write({
          ...
          error: {
            code: -32601,
            message: `Method not supported by this client: ${message.method}`,
          },
        });
      return;
    }
  ...
  interrupt(): void {
    ...
    // Cancellation is a hard lifecycle boundary. Killing the owned app-server guarantees that a
    // wedged JSON-RPC request cannot retain the turn; the next send lazily starts a fresh server.
