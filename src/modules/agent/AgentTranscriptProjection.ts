...
class $AgentTranscriptProjection {
  ...
  protected static projectEntry(
    ...
    const transcriptIcons =
      ThemeIcons.Class.agentTranscriptIconsFor(glyphLevel);
    const caret = {
      collapsed: transcriptIcons.caretCollapsed,
      expanded: transcriptIcons.caretExpanded,
    };
    const blank = (): void => {
      lines.push({
        text: '',
    ...
    };
    {
      ...
      switch (entry.role) {
        ...
        case 'assistant':
          ...
          lines.push({
            text: this.producerLabel(entry.engine),
            color: palette.func,
            bold: true,
            entryIndex,
            toggleable: false,
        ...
        case 'tool-use': {
          ...
          if (!expanded) {
            ...
            const summary = AgentToolSummary.Class.summarize(
              entry.name,
              entry.input,
            );
            const oneLine = summary.length > 0 ? `${head}  ${summary}` : head;
            lines.push({
              text: this.truncate(oneLine, width, glyphLevel),
              color: palette.type,
              bold: true,
              entryIndex,
              toggleable: true,
            ...
            lines.push({
              text: this.truncate(head, width, glyphLevel),
              color: palette.type,
              bold: true,
              entryIndex,
              toggleable: true,
            });
            for (const wrapped of this.wrap(
              this.toolInputPretty(entry.input),
              width,
            ))
              lines.push({
                text: wrapped,
        ...
        case 'tool-result': {
          ...
          if (!expanded) {
            ...
            const summary = AgentToolSummary.Class.summarizeResult(
              entry.result,
              entry.isError,
            );
            lines.push({
              text: this.truncate(
                `${marker} ${outcome} ${summary}`,
                width,
                glyphLevel,
              ),
              color,
  ...
  static project(
    ...
    transcript.forEach((entry, entryIndex) => {
      ...
      if (
        ...
        this.$entryProjectionCache.set(entry, {
          width,
          glyphLevel,
          palette,
          expanded,
    ...
    if (lines.length === 0)
      lines.push({
        text: this.emptyGreeting(activeProviderLabel),
        color: palette.dim,
        bold: false,
        entryIndex: -1,
        toggleable: false,
