...
class $TranscriptContextSerializer {
  static get DEFAULT_BUDGET_CHARACTERS(): number {
    return 4000;
  }
  ...
  protected static clip(text: string, limit: number): string {
    ...
    return codePoints.length <= limit
      ? collapsed
      : codePoints.slice(0, Math.max(0, limit - 1)).join('') + '…';
  }
    ...
    switch (entry.role) {
      ...
      case 'assistant':
        return entry.text.trim()
          ? `Assistant: ${this.clip(entry.text, 800)}`
          : null;
      case 'tool-use': {
        const input =
  ...
  static serialize(
    ...
    budgetCharacters = this.DEFAULT_BUDGET_CHARACTERS,
  ): string {
    const lines: string[] = [];
    ...
    const header =
      '[Context ported from the previous engine — the conversation so far:]';
    const body = (elided ? ['(…earlier turns elided…)', ...kept] : kept).join(
      '\n',
    );
    const footer =
      '[End of ported context. Continue the conversation seamlessly.]';
    return `${header}\n${body}\n${footer}`;
  }
}
