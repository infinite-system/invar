...
describe('AgentToolSummary.summarize — human phrases, never raw JSON', () => {
  test('Read/Write/Edit show the file BASENAME, not the full path', () => {
    expect(
      summarize('Read', { file_path: '/home/me/project/src/Foo.ts' }),
    ).toBe('Reading Foo.ts');
    expect(summarize('Write', { file_path: 'a/b/Bar.md' })).toBe(
      'Writing Bar.md',
    );
    expect(summarize('Edit', { file_path: 'x\\y\\Baz.tsx' })).toBe(
      'Editing Baz.tsx',
    );
    expect(summarize('MultiEdit', { file_path: '/q/Qux.js' })).toBe(
      'Editing Qux.js',
    );
  });
  ...
  test('Grep / Glob / LS / WebFetch / WebSearch / Task / TodoWrite each read their salient arg', () => {
    expect(summarize('Grep', { pattern: 'TODO', path: '/repo/src' })).toBe(
    ...
    expect(summarize('WebFetch', { url: 'https://example.com/a/b?c=1' })).toBe(
      'Fetching example.com',
    );
    expect(summarize('WebSearch', { query: 'how to reduce' })).toBe(
      'Searching "how to reduce"',
    );
    expect(
      summarize('Task', {
  ...
  test('an unknown tool degrades to its FIRST string arg (compact), never the whole JSON', () => {
    const phrase = summarize('MysteryTool', {
      count: 3,
      label: 'the salient value',
      nested: { a: 1 },
    });
