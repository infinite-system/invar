...
class $PngDecoder {
  ...
  protected static channelsForColorType(colorType: number): number {
    switch (colorType) {
      case 0:
        return 1; // grayscale
      case 2:
  ...
  protected static paethPredictor(
    left: number,
    above: number,
    upperLeft: number,
  ): number {
    ...
    if (
      ...
      distanceToLeft <= distanceToUpperLeft
    ) {
      return left;
    }
  ...
  protected static unfilterScanlines(
    inflated: Uint8Array,
    width: number,
    height: number,
      ...
      for (let byteIndex = 0; byteIndex < stride; byteIndex++) {
        ...
        const leftByte =
          byteIndex >= bytesPerPixel
            ? raw[rawRowStart + byteIndex - bytesPerPixel]!
            : 0;
        const aboveByte =
          rowIndex > 0 ? raw[rawRowStart - stride + byteIndex]! : 0;
        const upperLeftByte =
          rowIndex > 0 && byteIndex >= bytesPerPixel
            ? raw[rawRowStart - stride + byteIndex - bytesPerPixel]!
            : 0;
        ...
        switch (filterType) {
          case 0:
            ...
            break; // None
          case 1:
            reconstructedByte = filteredByte + leftByte;
            break; // Sub
          case 2:
  ...
  protected static expandToRgba(
    ...
    for (let pixelIndex = 0; pixelIndex < pixelCount; pixelIndex++) {
      ...
      if (colorType === 0) {
        ...
        if (!palette) {
          throw new Error('PNG: palette colour type without a PLTE chunk');
        }
        ...
        rgba[targetOffset + 3] =
          paletteAlpha && paletteIndex < paletteAlpha.length
  ...
  static decode(bytes: Uint8Array): DecodedImage {
    const pngSignature = this.$pngSignature;
    ...
    if (bitDepth !== 8) {
      throw new Error(
        `PNG: unsupported bit depth ${bitDepth} (only 8-bit supported)`,
      );
    }
    if (interlaceMethod !== 0) {
      throw new Error('PNG: interlaced images are not supported');
    }
    if (idatSegments.length === 0) {
      throw new Error('PNG: no image data (IDAT) found');
    }

    const channels = this.channelsForColorType(colorType);
    const compressedLength = idatSegments.reduce(
