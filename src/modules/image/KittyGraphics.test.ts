...
function parseApcSequences(
  payload: string,
): Array<{ controls: string; payload: string }> {
...
function solidImage(
  ...
  for (let offset = 0; offset < rgba.length; offset++)
    rgba[offset] = (channelSeed + offset * 7) & 0xff;
...
describe('KittyGraphics', () => {
  test('a small image emits ONE transmit-and-display APC whose payload round-trips to the RGBA', () => {
    ...
    const emitted = KittyGraphics.Class.place({
      image,
      imageId: 7001,
      columns: 8,
      rows: 2,
    ...
    const roundTripped = new Uint8Array(
      inflateSync(Buffer.from(sequences[0]!.payload, 'base64')),
    );
  ...
  test('a large image chunks at the 4096-byte base64 ceiling with honest m= flags and round-trips', () => {
    ...
    for (const sequence of sequences) {
      expect(sequence.payload.length).toBeLessThanOrEqual(
        KittyGraphics.Class.CHUNK_LIMIT,
      );
    }
