    ...
    for (let byteIndex = 0; byteIndex < stride; byteIndex++) {
      ...
      const left =
        byteIndex >= bytesPerPixel
          ? raw[rowIndex * stride + byteIndex - bytesPerPixel]!
          : 0;
      const above =
        rowIndex > 0 ? raw[(rowIndex - 1) * stride + byteIndex]! : 0;
      const upperLeft =
        rowIndex > 0 && byteIndex >= bytesPerPixel
          ? raw[(rowIndex - 1) * stride + byteIndex - bytesPerPixel]!
          : 0;
      let predictor = 0;
      switch (filterType) {
        case 0:
...
describe('PngDecoder', () => {
  test('RGB (colour type 2) round-trips through every scanline filter', () => {
    ...
    for (let index = 0; index < raw.length; index++)
      raw[index] = (index * 37 + 11) & 0xff; // varied gradient
  ...
  test('grayscale (colour type 0) expands each sample to opaque R=G=B', () => {
    ...
    const decoded = PngDecoder.Class.decode(
      encodePng({
        width: 4,
        height: 1,
        colorType: 0,
  ...
  test('palette (colour type 3) with tRNS resolves index → PLTE colour + per-index alpha', () => {
    ...
    const decoded = PngDecoder.Class.decode(png);
    expect([...decoded.rgba]).toEqual([
      255, 0, 0, 10, 0, 255, 0, 128, 0, 0, 255, 255,
    ]);
  });
  ...
  test('rejects a non-PNG buffer', () => {
    expect(() =>
      PngDecoder.Class.decode(Uint8Array.from([1, 2, 3, 4, 5, 6, 7, 8])),
