...
class $HalfBlockRenderer {
  ...
  protected static parseHexColor(hex: string): [number, number, number] {
  ...
  static render(context: HalfBlockRenderContext): HalfBlockRender {
    ...
    const [backgroundRed, backgroundGreen, backgroundBlue] =
      this.parseHexColor(panelBackground);
    const panelHex = this.rgbToHex(
      backgroundRed,
      backgroundGreen,
      backgroundBlue,
    );
