...
function colorOf(colorBuffer: {
  buffer: Record<number, number>;
}): [number, number, number] {
  return [
    colorBuffer.buffer[0]!,
    colorBuffer.buffer[1]!,
    colorBuffer.buffer[2]!,
  ];
}
...
describe('HalfBlockRenderer', () => {
  test('a 1×2 image maps top pixel → foreground, bottom pixel → background of one ▀ cell', () => {
    ...
    const render = HalfBlockRenderer.Class.render({
      image,
      columns: 1,
      rows: 1,
  ...
  test('a wide image in a tall pane is letterboxed with panel-background cells', () => {
    ...
    const image: DecodedImage = {
      width: 4,
      height: 1,
      rgba: new Uint8Array(4 * 4).fill(255),
    };
    const render = HalfBlockRenderer.Class.render({
      image,
      columns: 4,
    ...
    const imageCells = chunks.filter(
      (chunk) =>
        chunk.text.includes('▀') &&
        (colorOf(chunk.fg).join() === [255, 255, 255].join() ||
          colorOf(chunk.bg).join() === [255, 255, 255].join()),
    );
  ...
  test('never exceeds the requested pane and preserves aspect (no upscale past columns×2·rows)', () => {
    const image: DecodedImage = {
      width: 100,
      height: 50,
      rgba: new Uint8Array(100 * 50 * 4).fill(200),
    };
    const render = HalfBlockRenderer.Class.render({
      image,
      columns: 40,
