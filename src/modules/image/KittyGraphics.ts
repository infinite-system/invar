...
class $KittyGraphics {
  ...
  protected static apc(controls: string, payload: string): string {
  ...
  static place(placement: KittyPlacement): string {
  ...
  static remove(imageId: number): string {
    return this.apc(`a=d,d=I,i=${imageId},q=2`, '');
  }

  /** Delete every visible placement and its data — the app-dispose sweep. */
  static removeAll(): string {
    return this.apc('a=d,d=A,q=2', '');
  }
}
