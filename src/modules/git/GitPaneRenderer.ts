...
//
// GitPaneContent owns the hit-test geometry and asks this pure Static capability to project it.
// render() RETURNS the pane geometry (changesTop/dividerRow/…) rather than mutating closure state,
// so nothing here holds state.
//
...
class $GitPaneRenderer {
  protected static changeRowText(
    ...
    const checkbox =
      row.bucket === 'staged' ? checkboxIcons.checked : checkboxIcons.unchecked;
  ...
  protected static commitLogRowText(row: CommitLogRow): string {
    if (row.kind === 'commit') {
      ...
      return row.record
        ? ` ${chevron} ${row.record.shortSha} ${row.record.subject}`
        : ' …';
    }
    if (row.kind === 'loading') return '      …loading';
    return `    ${row.glyph} ${row.path}`;
  }
  public static changesContentWidth(
  ...
  public static logContentWidth(workspace: GitWorkspace.Instance): number {
    let widestWidth = 0;
  ...
  public static render(context: GitPaneRenderContext): {
    ...
    const emptyGeometry: GitPanelGeometry = {
      changesTop: 0,
      changesRows: 0,
      dividerRow: 0,
      logHeaderRow: -1,
    ...
    const git = workspace.repository.value;
    const gitPanel = workspace.panel;
    const active = workspace.workspace.focus.value === 'primaryPane';
    const pendingDiscard = gitPanel.confirmDiscard.value;
    if (pendingDiscard) {
      const label =
        pendingDiscard.paths.length === 1
          ? ` Discard changes to ${pendingDiscard.paths[0]}? [y/N]`
          : ` Discard changes to ${pendingDiscard.paths.length} files? [y/N]`;
      return {
        text: new StyledText([fg(palette.deleted)(label)]),
        geometry: emptyGeometry,
      };
    }
    if (!git)
      return {
        text: new StyledText([fg(palette.dim)('  no repository')]),
        geometry: emptyGeometry,
      };
    ...
    const changesActiveNameWidth = Math.max(
      1,
      changesViewportWidth - gitActionAreaWidth,
    );
    const logViewportWidth = Math.max(1, innerWidth - scrollbarThickness);
    pushRow(
      ` ${git.branch.value || '(no branch)'}  ${git.head.value.slice(0, 7)}`,
      palette.accent,
    );
    if (git.error.value) {
    ...
    const changeRows = GitRows.Class.buildChangeRows(
      git.staged.value,
      git.unstaged.value,
      git.untracked.value,
    );
    const topHeight = Math.max(
      2,
      Math.floor(bodyHeight * workspace.splitRatio),
    );
      ...
      glyph === 'A'
        ...
        : glyph === 'D'
          ...
          : glyph === '?'
            ...
            : palette.modified;
    changeRows
      .slice(changesTop, changesTop + changesVisible)
      .forEach((row, visibleIndex) => {
        ...
        if (row.kind === 'header') {
          ...
          const selectionFocused =
            active && gitPanel.region.value === 'changes';
    ...
    const changesRendered = Math.min(
      changeRows.length - changesTop,
      changesVisible,
    );
    ...
    const logHeight = Math.max(
      1,
      bodyHeight - topHeight - 1 - (commitLog ? 1 : 0),
    );
    if (commitLog) {
      ...
      const branchLabel = followingHead
        ? git.branch.value || 'HEAD'
        : `${viewedBranch} (view only)`;
      pushRow(
        ` history: ${branchLabel} ▾`,
        followingHead ? palette.dim : palette.modified,
      );
      const flatTop = gitPanel.logScrollTop.value;
      const expandedEntries =
        workspace.commitExpansion.value?.entries.value ?? [];
      ...
      flatRows.forEach((row, index) => {
        ...
        if (row.kind === 'commit') {
          pushRow(
            this.commitLogRowText(row),
            row.record ? palette.fg : palette.dim,
            {
              background,
              newline,
