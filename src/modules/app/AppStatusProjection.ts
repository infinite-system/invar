...
function $snapshot(ports: AppStatusProjectionPorts): Partial<StatusSnapshot> {
  ...
  return {
    ...
    workspaces: ports.workspaceSet.tabs().map((workspaceTab) => workspaceTab.name),
    ...
    lspSizeSuppressed: ports.workspaceSet.active.languageSizeNotice() !== null,
    ...
    findCurrentMatchIndex: ports.findBar.engine?.currentMatchIndex.value ?? -1,
    findCaseSensitive: ports.findBar.caseSensitive,
    sourceFindQuery: editor.hasDocument.value
      ? ports.findBar.engineFor(`source:${editor.document.path}`)?.query.value ?? ''
      : '',
    markdownPreviewFindQuery: markdownSplitView
      ? ports.findBar.engineFor(markdownSplitView.previewFindTargetIdentifier())?.query.value ?? ''
      : '',
    ...
    paletteMatches: ports.commands.open.value ? ports.commands.filtered.length : 0,
    ...
    settingsSelectedLabel: ports.settingsPanel.open.value
      ? (ports.settingsPanel.rows()[ports.settingsPanel.selectedIndex.value]?.label ?? '')
      : '',
    settingsSelectedValue: ports.settingsPanel.open.value
      ? (ports.settingsPanel.rows()[ports.settingsPanel.selectedIndex.value]?.valueText ?? '')
      : '',
    ...
    changesScrollTop: ports.workspaceSet.active.gitPanel.changesScrollTop.value,
    ...
    gitLogExpanded:
      ports.workspaceSet.active.commitExpansion.value?.entries.value.length ?? 0,
    ...
    gitLogBranch: ports.workspaceSet.active.commitLog.value?.branch.value ?? '',
    gitLogTipSha: ports.workspaceSet.active.commitLog.value?.loadedTipSha ?? '',
    ...
    panelCellIds: ports.panelHost.resolvedCells.map((cell) => cell.content.id),
    ...
    currentLineBlameAuthor: ports.workspaceSet.active.activeLineBlame?.author ?? '',
    ...
    matchingBracketLine: (() => {
      if (!editor.hasDocument.value || ports.workspaceSet.active.showingDiff.value) return -1;
      return (
