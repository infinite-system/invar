...
function query(
  ...
  cursorLine: number,
  cursorColumn: number,
  options?: Partial<BracketMatchQuery>,
): BracketMatchQuery {
...
test('cursor ON an opener matches its closer', () => {
  expect(BracketMatch.Class.find(query(['(a + b)'], 0, 0))).toEqual({
    bracket: { line: 0, column: 0 },
...
test('cursor immediately AFTER a closer matches its opener', () => {
  ...
  expect(BracketMatch.Class.find(query(['(a)'], 0, 3))).toEqual({
    bracket: { line: 0, column: 2 },
...
test('nested same-family brackets balance by depth', () => {
  ...
  expect(BracketMatch.Class.find(query(['((()))'], 0, 1))?.match).toEqual({
    line: 0,
    column: 4,
  });
});
...
test('different families nest transparently', () => {
  ...
  expect(BracketMatch.Class.find(query(['{[()]}'], 0, 0))?.match).toEqual({
    line: 0,
    column: 5,
  });
});
...
test('matches across multiple lines', () => {
  ...
  expect(BracketMatch.Class.find(query(lines, 0, 13))?.match).toEqual({
    line: 2,
    column: 0,
  });
});
...
test('a bracket the predicate rejects (string/comment) is skipped during the scan', () => {
  ...
  expect(
    BracketMatch.Class.find(query(['( ) )'], 0, 0, { isCodeBracket }))?.match,
  ).toEqual({ line: 0, column: 4 });
});
...
test('a cursor bracket the predicate rejects is not matched at all', () => {
  ...
  expect(
    BracketMatch.Class.find(query(['(a)'], 0, 0, { isCodeBracket })),
  ).toBeNull();
});
...
test('the scan cap bounds a pathological unbalanced file (no hang, no match)', () => {
  ...
  expect(
    BracketMatch.Class.find(query(lines, 0, 0, { maxScanCells: 10 })),
  ).toBeNull();
});
...
test('findInDocument skips a bracket inside a string via the real tokenizer', () => {
  ...
  const result = BracketMatch.Class.findInDocument(
    document,
    0,
    1,
    'typescript',
...
test('findInDocument reuses one revision/cursor/language snapshot across frames', () => {
  ...
  document.loadFromText(
    ['{', ...Array.from({ length: 500 }, () => '  "value": {}'), '}'].join(
      '\n',
    ),
    'fixture.json',
  );
  const originalLine = document.line.bind(document);
  let lineReads = 0;
  document.line = (lineIndex: number): string => {
    lineReads++;
    return originalLine(lineIndex);
  };

  const first = BracketMatch.Class.findInDocument(document, 0, 0, 'json');
  const readsAfterFirstProjection = lineReads;
  expect(first?.match.line).toBe(501);
  expect(readsAfterFirstProjection).toBeGreaterThan(0);

  for (let frameNumber = 0; frameNumber < 10_000; frameNumber++) {
    expect(BracketMatch.Class.findInDocument(document, 0, 0, 'json')).toBe(
      first,
    );
  }
  expect(lineReads).toBe(readsAfterFirstProjection);

  document.setLine(1, '  "changed": {}');
  BracketMatch.Class.findInDocument(document, 0, 0, 'json');
  expect(lineReads).toBeGreaterThan(readsAfterFirstProjection);
});
