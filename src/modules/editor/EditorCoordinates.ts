...
class $EditorCoordinates {
  ...
  // Memoized segmentation, keyed by line CONTENT (content-keyed = revision-proof: an edited line is
  // a new string; identical lines share an entry). Repeated coordinate lookups during a selection
  // drag or repaint re-segment nothing — the walk cost is paid once per distinct line. Bounded: on
  // overflow the oldest half is dropped (insertion order).
  protected static memoized<Value>(
  ...
  static graphemeBoundaries(line: string): number[] {
    return this.memoized(this.$boundariesMemo, line, () => {
  ...
  static graphemes(line: string): string[] {
    return this.memoized(this.$clustersMemo, line, () =>
      TextSegmentation.Class.graphemes(line),
    );
  }

  /** Number of user-perceived characters (grapheme clusters) in a line. */
  static graphemeCount(line: string): number {
    return this.graphemeBoundaries(line).length - 1;
  }

  /** UTF-16 offset of the start of grapheme `g` (clamped to [0, count]). */
  static graphemeToU16(line: string, graphemeIndex: number): number {
    const boundaries = this.graphemeBoundaries(line);
  ...
  static u16ToGrapheme(line: string, utf16Offset: number): number {
    const boundaries = this.graphemeBoundaries(line);
    let graphemeIndex = 0;
  ...
  static codePointWidth(codePoint: number): number {
  ...
  static graphemeWidth(grapheme: string): number {
    let width = 0;
    for (const character of grapheme) {
      const codePoint = character.codePointAt(0);
      if (codePoint === undefined) continue;
      const characterWidth = this.codePointWidth(codePoint);
  ...
  protected static displayColumnPrefix(line: string, tabWidth = 4): number[] {
    const build = (): number[] => {
    ...
    return tabWidth === 4
      ? this.memoized(this.$displayPrefixMemo, line, build)
      : build();
  }
  ...
  static lineWidth(line: string, tabWidth = 4): number {
    ...
    return tabWidth === 4
      ? this.memoized(this.$lineWidthMemo, line, compute)
      : compute();
  }
   ...
   */
  static graphemeAtDisplayColumn(
    line: string,
    targetColumn: number,
    tabWidth = 4,
  ): number {
  ...
  static clampCol(line: string, column: number): number {
    return Math.max(0, Math.min(column, this.graphemeCount(line)));
  }
   ...
   */
  static displayColumnWindow(
    text: string,
    scrollLeft: number,
    viewportWidth: number,
  ): string {
    const safeViewportWidth = Math.max(1, viewportWidth);
    if (scrollLeft <= 0 && this.lineWidth(text) <= safeViewportWidth)
      return text;
    let startGrapheme = this.graphemeAtDisplayColumn(
      text,
      Math.max(0, scrollLeft),
    );
    if (this.displayColumn(text, startGrapheme) < scrollLeft)
      startGrapheme += 1;
    let endGrapheme =
      this.graphemeAtDisplayColumn(
        text,
        Math.max(0, scrollLeft) + safeViewportWidth,
      ) + 1;
    let windowText = text.slice(
      this.graphemeToU16(text, startGrapheme),
      this.graphemeToU16(text, endGrapheme),
    );
    while (
      endGrapheme > startGrapheme &&
      this.lineWidth(windowText) > safeViewportWidth
    ) {
      endGrapheme -= 1;
  ...
  static wordBounds(
    ...
    const lineGraphemes = this.graphemes(line);
