 ...
 */
function wrapLineImplementation(lineText: string, wrapWidth: number): WrapSegment[] {
...
function visualRowCountImplementation(lineText: string, wrapWidth: number): number {
  return wrapLineImplementation(lineText, wrapWidth).length;
}
 ...
 */
function segmentIndexForCursorImplementation(segments: WrapSegment[], graphemeColumn: number): number {
 ...
 */
function visualRowsForWindowImplementation(
  ...
  for (let lineIndex = firstLine; lineIndex < document.lineCount && rows.length < height; lineIndex++) {
    const segments = wrapLineImplementation(document.line(lineIndex), wrapWidth);
 ...
 * last visual rows.
 */
function moveByVisualRowsImplementation(
  document: WrappableDocument,
  ...
  let segments = wrapLineImplementation(document.line(lineIndex), wrapWidth);
  let segmentIndex = segmentIndexForCursorImplementation(segments, position.col);
  ...
  while (remaining > 0) {
    if (segmentIndex + 1 < segments.length) {
      ...
      lineIndex += 1;
      segments = wrapLineImplementation(document.line(lineIndex), wrapWidth);
      segmentIndex = 0;
    } else {
      break;
    }
 ...
 */
function scrollTopToRevealCursorImplementation(
  document: WrappableDocument,
  ...
  for (let lineIndex = top; lineIndex < cursorLine; lineIndex++) {
    rowsThroughCursor += visualRowCountImplementation(document.line(lineIndex), wrapWidth);
  }
  while (rowsThroughCursor > height && top < cursorLine) {
    rowsThroughCursor -= visualRowCountImplementation(document.line(top), wrapWidth);
    top += 1;
  }
  return top;
}
