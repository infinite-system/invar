class $SelectionDragBehavior {
  ...
  begin(screenColumn: number, screenRow: number): void {
    ...
    this.options.beginSelection(
      position,
      this.pointerDisplayColumn(screenColumn),
    );
  }
  drag(screenColumn: number, screenRow: number): void {
    ...
    const position = this.options.positionAtCell(screenColumn, screenRow);
    if (!position) return;
    this.options.extendSelection(
      this.inclusiveHead(position),
      this.pointerDisplayColumn(screenColumn),
    );
  }
  end(): void {
  ...
  protected inclusiveHead(
    position: SelectionDragPosition,
  ): SelectionDragPosition {
  ...
  tick(deltaTimeSeconds: number): boolean {
    ...
    if (this.options.horizontalScrollingEnabled() && columnOvershoot !== 0) {
      this.columnScrollRemainder +=
        this.cellsPerSecond(columnOvershoot) * deltaTimeSeconds;
    } else {
      this.columnScrollRemainder = 0;
    }
    this.rowScrollRemainder +=
      rowOvershoot === 0
        ? 0
    ...
    const position = this.options.positionAtCell(
      clampedScreenColumn,
      clampedScreenRow,
    );
    if (position) {
      this.options.extendSelection(
        this.inclusiveHead(position),
        this.pointerDisplayColumn(clampedScreenColumn),
      );
    }
    return true;
  }
  protected pointerDisplayColumn(screenColumn: number): number {
    ...
    return Math.max(
      0,
      this.options.horizontalScrollPosition() +
        screenColumn -
        viewportRectangle.leftColumn,
    );
  }
