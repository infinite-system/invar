class $SelectionDragBehavior {
    ...
    protected inclusiveHead(position: SelectionDragPosition): SelectionDragPosition {
        const anchor = this.selectionAnchor;
        const lineGraphemeCount = this.options.lineGraphemeCount;
        if (!anchor || !lineGraphemeCount)
            return position;
        const headAtOrAfterAnchor = position.line > anchor.line ||
            (position.line === anchor.line && position.column >= anchor.column);
        if (!headAtOrAfterAnchor)
            return position;
        return {
            line: position.line,
    ...
    tick(deltaTimeSeconds: number): boolean {
        ...
        const columnOvershoot = this.overshoot(pointerPosition.screenColumn, viewportRectangle.leftColumn, viewportRectangle.rightColumn);
        const rowOvershoot = this.overshoot(pointerPosition.screenRow, viewportRectangle.topRow, viewportRectangle.bottomRow);
        ...
        if (columnStep !== 0)
            this.options.scrollColumns(columnStep);
        if (rowStep !== 0)
            this.options.scrollRows(rowStep);
        const clampedScreenColumn = Math.max(viewportRectangle.leftColumn, Math.min(pointerPosition.screenColumn, viewportRectangle.rightColumn));
        const clampedScreenRow = Math.max(viewportRectangle.topRow, Math.min(pointerPosition.screenRow, viewportRectangle.bottomRow));
    ...
    protected pointerDisplayColumn(screenColumn: number): number {
        const viewportRectangle = this.options.viewportRectangle();
        return Math.max(0, this.options.horizontalScrollPosition() + screenColumn - viewportRectangle.leftColumn);
    }
    protected overshoot(pointerCoordinate: number, minimumCoordinate: number, maximumCoordinate: number): number {
        if (pointerCoordinate >= maximumCoordinate)
            return pointerCoordinate - maximumCoordinate + 1;
        if (pointerCoordinate <= minimumCoordinate)
            return pointerCoordinate - minimumCoordinate - 1;
        return 0;
    }
    protected cellsPerSecond(overshoot: number): number {
