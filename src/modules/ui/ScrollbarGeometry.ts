...
class $ScrollbarGeometry {
  ...
  public static scrollbarGeometry(
    ...
    const trackLeft =
      orientation === 'vertical' ? region.left + region.width - 1 : region.left;
    let reportedViewportSize = scroll.viewportSize;
    const minimumViewportForThumb = Math.ceil(
      (minimumThumbCells * scroll.scrollSize) / trackLength,
    );
    reportedViewportSize = Math.min(
      scroll.scrollSize,
      Math.max(reportedViewportSize, minimumViewportForThumb),
    );
    ...
    const reportedToTrueScale =
      reportedRange > 0 ? trueRange / reportedRange : 0;
    const clampedPosition = Math.max(
      0,
      Math.min(scroll.scrollPosition, trueRange),
    );
    const reportedPosition =
      trueRange > 0
        ? Math.round((clampedPosition / trueRange) * reportedRange)
        : 0;
    return {
      trackTop,
      trackLeft,
