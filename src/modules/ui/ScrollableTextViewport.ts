...
class $ScrollableTextViewport {
  ...
  constructor(protected readonly deps: ScrollableTextViewportDeps) {
    ...
    this.drag = new SelectionDragBehavior.Class({
      ...
      positionAtCell: (screenColumn, screenRow) =>
        deps.selection.positionAtCell(screenColumn, screenRow),
      ...
      beginSelection: (position, pointerDisplayColumn) =>
        deps.selection.begin(position, pointerDisplayColumn),
      extendSelection: (position, pointerDisplayColumn) =>
        deps.selection.extend(position, pointerDisplayColumn),
  ...
  get stuckToBottom(): boolean {
    return this.followBottomActive;
  }
  ...
  followContentTail(): void {
    if (this.deps.followBottom && this.followBottomActive)
  ...
  protected verticalMomentumOptions(): MomentumOptions {
  ...
  tick(deltaSeconds: number): boolean {
    let keepAlive = false;
    const vertical = Momentum.Class.stepMomentum(
      this.verticalMomentum,
      deltaSeconds,
      this.verticalMomentumOptions(),
    );
    ...
    const horizontal = Momentum.Class.stepMomentum(
      this.horizontalMomentum,
      deltaSeconds,
      Momentum.Class.defaultOptions,
    );
  ...
  protected setScrollLeft(value: number): void {
    const clamped = Math.max(0, Math.min(value, this.maximumScrollLeft()));
    if (clamped !== this.scrollLeftValue) {
      this.scrollLeftValue = clamped;
      this.deps.onScroll();
    }
    if (clamped === 0 || clamped === this.maximumScrollLeft())
      this.horizontalMomentum = Momentum.Class.halt();
  }
  // Pointer selection lifecycle — the host wires its content renderable's mouse events to these.
  beginDrag(screenColumn: number, screenRow: number): void {
    this.drag.begin(screenColumn, screenRow);
  }
  dragTo(screenColumn: number, screenRow: number): void {
    this.drag.drag(screenColumn, screenRow);
  }
  endDrag(): void {
    this.drag.end();
  }
  ...
  updateScrollbars(region: ViewportRegion): void {
    ...
    const thickness = Math.max(
      1,
      Math.round(this.deps.settings.scrollbarThickness.value),
    );
    ...
    const horizontalOverflow =
      !this.deps.disableHorizontal &&
      extent.contentColumns > extent.viewportColumns;
    const rect = {
      top: region.top,
      left: region.left,
      width: region.width,
    ...
    if (verticalOverflow) {
      const geometry = ScrollbarGeometry.Class.scrollbarGeometry(
        ...
        rect,
        {
          scrollSize: extent.contentRows,
          viewportSize: extent.viewportRows,
          scrollPosition: this.scrollTopValue,
        },
      );
      if (geometry) {
    ...
    if (horizontalOverflow) {
      const geometry = ScrollbarGeometry.Class.scrollbarGeometry(
        ...
        rect,
        {
          scrollSize: extent.contentColumns,
          viewportSize: extent.viewportColumns,
          scrollPosition: this.scrollLeftValue,
        },
      );
      if (geometry) {
