...
class $ScrollableTextViewport {
    ...
    tick(deltaSeconds: number): boolean {
        ...
        if (Momentum.Class.isMoving(this.horizontalMomentum))
            keepAlive = true;
        if (this.drag.active)
            keepAlive = this.drag.tick(deltaSeconds) || keepAlive;
        return keepAlive;
    }
    ...
    protected setScrollTop(value: number): void {
        ...
        if (this.deps.followBottom)
            this.followBottomActive = clamped >= maximum;
    }
