...
class $EditorPaneRenderer {
    ...
    public static render(context: EditorPaneRenderContext): EditorPaneRender | null {
        const { workspace, palette } = context;
        if (workspace.showingDiff.value)
            return null;
        ...
        const severityColor = (severity: number): string => severity === 1 ? palette.error : severity === 2 ? palette.warning : palette.info;
        const mostSevere = (marks: readonly {
            severity: number;
        }[]): number => marks.reduce((worst, mark) => Math.min(worst, mark.severity), 4);
        ...
        const pushCodeChunks = (windowText: string, lineIndex: number, windowStartGrapheme = 0, windowSpans: readonly Span[] | null = null): void => {
            ...
            if (context.showIndentGuides && windowStartGrapheme === 0) {
                for (let indentGrapheme = 0; indentGrapheme < windowGraphemeCount; indentGrapheme += 1) {
                    if (windowText[indentGrapheme] !== ' ')
                        break;
                    if (EditorCoordinates.Class.displayColumn(windowText, indentGrapheme) % this.indentGuideTabWidth === 0) {
            ...
            for (let boundaryIndex = 0; boundaryIndex < orderedBoundaries.length - 1; boundaryIndex += 1) {
                ...
                if (segmentEnd <= segmentStart)
                    continue;
                const segmentText = windowText.slice(EditorCoordinates.Class.graphemeToU16(windowText, segmentStart), EditorCoordinates.Class.graphemeToU16(windowText, segmentEnd));
                const findHighlighted = lineMatches.some((match) => match.startColumn < windowStartGrapheme + segmentEnd &&
                    match.endColumn > windowStartGrapheme + segmentStart);
                const bracketHighlighted = lineBracketColumns.some((bracketColumn) => bracketColumn >= windowStartGrapheme + segmentStart && bracketColumn < windowStartGrapheme + segmentEnd);
                const diagnosticSeverity = diagnosticSeverityOver(windowStartGrapheme + segmentStart, windowStartGrapheme + segmentEnd);
                if (indentGuideGraphemes.has(segmentStart) &&
                ...
                if (bracketHighlighted) {
                    ...
                    for (const span of Highlighter.Class.sliceSpans(windowSpans, segmentStart, segmentEnd)) {
                        const syntaxChunk = fg(this.roleColor(span.role, palette))(span.text);
        ...
        if (editor.wordWrap.value) {
            ...
            wrapRowsWindow.forEach((row, rowIndex) => {
                ...
                if (!plainForeground) {
                    ...
                    segmentSpans = Highlighter.Class.sliceSpans(tokenizedLineSpans, row.segment.startGrapheme, row.segment.endGrapheme);
                }
                pushCodeChunks(lineText.slice(EditorCoordinates.Class.graphemeToU16(lineText, row.segment.startGrapheme), EditorCoordinates.Class.graphemeToU16(lineText, row.segment.endGrapheme)), row.lineIndex, row.segment.startGrapheme, segmentSpans);
        ...
        visibleLines.forEach((text, visibleIndex) => {
            ...
            if (scrollLeft > 0 || text.length > viewportWidth) { // O(1) test; a needless slice is harmless
                ...
                if (EditorCoordinates.Class.displayColumn(text, startGrapheme) < scrollLeft)
