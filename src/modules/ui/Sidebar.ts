...
class $Sidebar {
    ...
    protected gitActionButtonAt(relativeX: number): GitActionButton | null {
        const innerWidth = this.deps.sidebarWidth() - 2;
        const actionAreaStart = Math.max(1, innerWidth - this.deps.scrollbarThicknessCells() - this.deps.gitActionAreaWidth);
        if (relativeX >= actionAreaStart && relativeX < actionAreaStart + 2)
            return 'open';
        if (relativeX >= actionAreaStart + 2 && relativeX < actionAreaStart + 5)
            return 'discard';
        if (relativeX >= actionAreaStart + 5 && relativeX < actionAreaStart + 8)
            return 'stageToggle';
        return null;
    }
    ...
    protected openChangesContextMenu(rowIndex: number, row: FileRow, rows: ChangeRow[], pointerX: number, pointerY: number): void {
        const { workspaceSet, overlayCoordinator, contextMenu, renderer } = this.deps;
        const gitPanel = workspaceSet.active.gitPanel;
        if (!gitPanel.selectedPaths.value.has(row.path))
            gitPanel.replaceSelected([row.path]);
        gitPanel.setChangesSelection(rowIndex);
        const selectedFileRows = rows.filter((candidate): candidate is FileRow => candidate.kind === 'file' && gitPanel.selectedPaths.value.has(candidate.path));
        ...
        const firstSelectedIndex = rows.findIndex((candidate) => candidate.kind === 'file' && gitPanel.selectedPaths.value.has(candidate.path));
        overlayCoordinator.openExclusiveOverlay('contextMenu', () => contextMenu.openAt(items, pointerX, pointerY, { width: renderer.width, height: renderer.height }, (itemId) => {
            if (itemId === 'git.stageSelected')
                void workspaceSet.active.stageSelected();
            else if (itemId === 'git.unstageSelected')
                void workspaceSet.active.unstageSelected();
            else if (itemId === 'git.discardSelected')
                workspaceSet.active.requestDiscardSelected(); // y/N confirm
            else if (itemId === 'git.openDiff' && firstSelectedIndex >= 0)
                void workspaceSet.active.openChangeAtRow(firstSelectedIndex);
        }));
    }
    ...
    protected openLogBranchMenu(pointerX: number, pointerY: number): void {
        ...
        void activeWorkspace.localLogBranches().then((branchNames) => {
            ...
            overlayCoordinator.openExclusiveOverlay('boundedListPopup', () => boundedListPopup.openAt(items, { column: pointerX, row: pointerY }, (item) => activeWorkspace.selectLogBranch(item.identifier), {
    ...
    protected selectChangesRange(anchorIndex: number, targetIndex: number, rows: ChangeRow[]): void {
    ...
    protected wireHandlers(): void {
        ...
        sidebar.onMouseScroll = (event) => {
            ...
            if (workspaceSet.active.sidebarView.value === 'git') {
                ...
                if (row < this.deps.gitPanelGeometry().dividerRow) {
                    if (horizontal)
                        workspaceSet.active.impulseGitChangesHorizontalScroll((backward ? -1 : 1) * step);
                    else
                        workspaceSet.active.impulseGitChangesScroll((direction === 'up' ? -1 : 1) * step);
                }
                else {
                    if (horizontal)
                        workspaceSet.active.impulseGitLogHorizontalScroll((backward ? -1 : 1) * step);
                    else
                        workspaceSet.active.impulseGitLog((direction === 'up' ? -1 : 1) * step);
                }
            }
            else if (horizontal) {
                this.deps.treeContent()?.onHorizontalWheel?.((backward ? -1 : 1) * step);
            }
            else {
                this.deps.treeContent()?.onWheel?.((direction === 'up' ? -1 : 1) * step);
            }
        };
        sidebar.onMouseMove = (event) => {
            if (workspaceSet.active.sidebarView.value === 'git') {
                ...
                const button = hoveredRow?.kind === 'file' ? this.gitActionButtonAt(event.x - (sidebar.x + 1)) : null;
            ...
            tooltip.clear();
            this.deps.treeContent()?.onPointerMove?.(event.x - (sidebar.x + 1), event.y - (sidebar.y + 1));
        };
        sidebar.onMouseOut = () => {
        ...
        sidebar.onMouseDown = (event) => {
            ...
            this.deps.treeContent()?.onPointerDown?.(event.x - (sidebar.x + 1), event.y - (sidebar.y + 1));
        };
    }
}
