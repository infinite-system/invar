...
import { TabBarRenderer, type TabBarSegment, type TabBarHover, type WorkspaceTabBarSegment, type WorkspaceTabBarHover, } from './TabBarRenderer';
...
class $TabBar {
    ...
    constructor(protected readonly deps: TabBarDeps) {
    ...
    protected workspaceSegmentAt(primaryCoordinate: number): WorkspaceTabBarSegment | null {
        return this.workspaceSegments.find((segment) => primaryCoordinate >= segment.primaryStart && primaryCoordinate < segment.primaryEnd) ?? null;
    }
    ...
    protected openTabDropdown(anchorColumn: number): void {
        ...
        overlayCoordinator.openExclusiveOverlay('boundedListPopup', () => boundedListPopup.openAt(items, {
    ...
    protected wireWorkspaceHandlers(): void {
        ...
        workspaceTabBar.onMouseDown = (event) => {
            ...
            if (segment.kind === 'tab') {
                ...
                if (closeHit && workspaceSet.count > 1)
                    ...
                    workspaceSet.activate(segment.workspaceIndex);
            }
                ...
                overlayCoordinator.openExclusiveOverlay('quickOpen', () => quickOpen.showWorkspacePath(workspaceSet.active.root));
            }
            renderer.requestRender();
        };
        workspaceTabBar.onMouseMove = (event) => {
            ...
            if (segment?.kind === 'tab') {
                ...
                tooltip.point(closeHit
                    ? 'Close project (Ctrl+Shift+W)'
                    : `Switch project: ${workspaceTab?.name ?? ''} (Ctrl+Shift+PageUp/PageDown)`, event.x, event.y);
            }
            else if (segment) {
                nextHover = { kind: segment.kind, workspaceIndex: -1 };
                tooltip.point(segment.kind === 'add'
                    ? 'Open project folder (Ctrl+Shift+O)'
                    : 'Pan project tabs without switching', event.x, event.y);
            }
            else {
                tooltip.clear();
            }
    ...
    protected wireBufferHandlers(): void {
        ...
        tabBar.onMouseDown = (event) => {
            ...
            if (segment.kind === 'tab') {
                if (localColumn === segment.closeColumn) {
                    ...
                    workspaceSet.active.activateTab(segment.index);
            }
        ...
        tabBar.onMouseMove = (event) => {
            ...
            if (segment?.kind === 'previewToggle') {
                ...
                tooltip.point(`Toggle Markdown preview${bindingHint ? ` (${bindingHint})` : ''}`, event.x, event.y);
            }
