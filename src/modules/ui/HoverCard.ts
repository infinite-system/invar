...
class $HoverCard {
  ...
  protected contentPositionAtCell(
    ...
    const start = Math.max(
      0,
      Math.min(this.viewport.scrollTop, this.maximumScrollTop()),
    );
    const row = Math.max(
      0,
      Math.min(
        start + (screenRow - this.content.y),
        this.contentLines.length - 1,
      ),
    );
    const column = Math.max(
      0,
      this.viewport.scrollLeft + (screenColumn - this.content.x),
    );
  ...
  protected maxInteriorRows(): number {
    return Math.max(
      1,
      Math.min(this.maxCardRows, this.deps.renderer.height - 4),
    );
  }
  protected viewportRows(): number {
    return Math.max(
      1,
      Math.min(this.contentLines.length, this.maxInteriorRows()),
    );
  }
  ...
  protected maxInteriorColumns(): number {
    return Math.max(
      1,
      Math.min(this.maxCardColumns, this.deps.renderer.width - 4),
    );
  }
  protected viewportColumns(): number {
    return Math.max(
      1,
      Math.min(this.contentMaxWidth, this.maxInteriorColumns()),
    );
  }
  ...
  pointAt(position: LanguagePosition, screenX: number, screenY: number): void {
    ...
    if (
      this.visible &&
      this.shownRange &&
      this.positionInRange(position, this.shownRange)
    ) {
      return;
    }
  ...
  tick(deltaSeconds: number): boolean {
    ...
    if (this.visible) {
      ...
      if (this.pointerOverCard || this.onSymbol || dragging) {
        ...
        const dismissLimit = this.cardWasEntered
          ? this.hoverIdleDismissSeconds
          : this.hoverSymbolOffDismissSeconds;
    ...
    if (this.pending) {
      ...
      if (
        this.dwellSeconds >= this.hoverDwellSeconds &&
        ...
        void this.deps.requestHover(requestPosition).then((hover) => {
          ...
          this.renderContents(
            hasHoverContents ? hover!.contents : '',
            diagnostics,
          );
  ...
  protected selectedText(): string {
    ...
    const startRow = Math.max(
      0,
      Math.min(start.row, this.contentPlain.length - 1),
    );
    const endRow = Math.max(0, Math.min(end.row, this.contentPlain.length - 1));
    if (startRow === endRow) {
      return (this.contentPlain[startRow] ?? '').slice(
        start.column,
        end.column,
      );
    }
    const parts: string[] = [
      (this.contentPlain[startRow] ?? '').slice(start.column),
    ];
  ...
  protected paintSelection(
    ...
    if (
      !span ||
      (span[0].row === span[1].row && span[0].column === span[1].column)
    ) {
    ...
    const toLocalColumn = (column: number) =>
      Math.max(0, Math.min(column - windowStartColumn, viewportColumns));
    const anchorY = Math.max(
      0,
      Math.min(start.row - windowStart, viewportRows - 1),
    );
    const anchorX = start.row >= windowStart ? toLocalColumn(start.column) : 0;
    const focusY = Math.max(
      0,
      Math.min(end.row - windowStart, viewportRows - 1),
    );
    const focusX =
      end.row <= windowEnd ? toLocalColumn(end.column) : viewportColumns;
  ...
  protected renderContents(
    ...
    for (const rawLine of markdown.replace(/\r\n/g, '\n').split('\n')) {
      ...
      if (insideFence) {
        ...
        const chunks = Highlighter.Class.highlightLine(rawLine, language).map(
          (span) => fg(this.roleColor(span.role, palette))(span.text),
        );
    ...
    this.contentPlain = this.contentLines.map((chunks) =>
  ...
  protected windowChunks(
    ...
    for (const chunk of lineChunks) {
      ...
      out.push({
        ...chunk,
        text: text.slice(from - chunkStart, to - chunkStart),
      });
    }
    ...
    return [
      base
        ? { ...base, text: ' ' }
        : ({ __isChunk: true, text: ' ' } as TextChunk),
    ];
  }
  ...
  update(palette: Palette): void {
    ...
    const left = Math.max(
      0,
      Math.min(
        this.anchorX + this.anchorOffsetColumns,
        Math.max(0, renderer.width - boxWidth),
      ),
    );
    ...
    const startColumn = Math.max(
      0,
      Math.min(this.viewport.scrollLeft, this.maximumScrollLeft()),
    );
    const chunks: TextChunk[] = [];
    const visibleLines = this.contentLines.slice(start, start + viewportRows);
    visibleLines.forEach((lineChunks, index) => {
      chunks.push(
        ...this.windowChunks(lineChunks, startColumn, interiorContentWidth),
      );
