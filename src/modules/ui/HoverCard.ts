...
import { BoxRenderable, StyledText, fg, type MouseEvent, type TextChunk, type CliRenderer, } from '@opentui/core';
...
class $HoverCard {
    ...
    protected static get $fenceLanguage(): Record<string, LangId> {
        ...
        Object.defineProperty(this, "$fenceLanguage", { configurable: true, value: fenceLanguageValue });
    ...
    protected contentLines: TextChunk[][] = [];
    /** The plain text of each content line (chunk texts joined) — the source for a copied selection. */
    protected contentPlain: string[] = [];
    protected rawContents: string | null = null;
    protected contentMaxWidth = 0;
    ...
    protected interiorColumns = 1;
    protected dwellSeconds = 0;
    ...
    protected requestedGeneration = -1;
    ...
    protected cardWasEntered = false;
    ...
    protected selectionAnchor: {
        row: number;
        column: number;
    } | null = null;
    protected selectionFocus: {
        row: number;
        column: number;
    } | null = null;
    /**
    ...
    protected readonly backdrop: HitTransparentText.Model;
    protected readonly box: BoxRenderable;
    protected readonly content: SelectableText.Model;
    ...
    protected contentPositionAtCell(screenColumn: number, screenRow: number): {
        line: number;
        column: number;
    } | null {
        if (!this.visible || this.contentLines.length === 0)
            return null;
    ...
    protected maxInteriorRows(): number {
        return Math.max(1, Math.min(this.maxCardRows, this.deps.renderer.height - 4));
    }
    protected viewportRows(): number {
        return Math.max(1, Math.min(this.contentLines.length, this.maxInteriorRows()));
    }
    protected maximumScrollTop(): number {
    ...
    protected maxInteriorColumns(): number {
        return Math.max(1, Math.min(this.maxCardColumns, this.deps.renderer.width - 4));
    }
    protected viewportColumns(): number {
        return Math.max(1, Math.min(this.contentMaxWidth, this.maxInteriorColumns()));
    }
    protected maximumScrollLeft(): number {
    ...
    tick(deltaSeconds: number): boolean {
        ...
        if (this.visible) {
            ...
            if (this.pointerOverCard || this.onSymbol || dragging) {
                ...
                const dismissLimit = this.cardWasEntered ? this.hoverIdleDismissSeconds : this.hoverSymbolOffDismissSeconds;
        ...
        if (this.pending) {
            ...
            if (this.dwellSeconds >= this.hoverDwellSeconds && this.requestedGeneration !== this.generation) {
                ...
                void this.deps.requestHover(requestPosition).then((hover) => {
                    if (capturedGeneration !== this.generation)
                        return; // stale: the pointer moved on
    ...
    protected selectedText(): string {
        ...
        for (let row = startRow + 1; row < endRow; row += 1)
            parts.push(this.contentPlain[row] ?? '');
    ...
    protected paintSelection(windowStart: number, viewportRows: number, windowStartColumn: number, viewportColumns: number): void {
        const span = this.normalizedSelection();
     ...
     */
    protected renderContents(markdown: string, diagnostics: readonly HoverDiagnostic[] = []): void {
        ...
        for (const diagnostic of diagnostics) {
            const severityColor = diagnostic.severity === 1 ? palette.error : diagnostic.severity === 2 ? palette.warning : palette.info;
            const severityLabel = diagnostic.severity === 1 ? 'error' : diagnostic.severity === 2 ? 'warning' : diagnostic.severity === 3 ? 'info' : 'hint';
        ...
        for (const rawLine of markdown.replace(/\r\n/g, '\n').split('\n')) {
            ...
            if (fenceMatch) {
                if (!insideFence) {
                    ...
                    fenceLanguage = this.fenceLanguage[tag] ?? null;
                }
                else {
                    insideFence = false;
                    fenceLanguage = null;
            ...
            if (insideFence) {
                ...
                const chunks = Highlighter.Class.highlightLine(rawLine, language).map((span) => fg(this.roleColor(span.role, palette))(span.text));
        ...
        while (lines.length) {
            const first = lines[0];
            if (!first || !this.isBlankLine(first))
                break;
            lines.shift();
        }
        while (lines.length) {
            const last = lines[lines.length - 1];
            if (!last || !this.isBlankLine(last))
                break;
            lines.pop();
        }
    ...
    protected isBlankLine(chunks: TextChunk[]): boolean {
    ...
    protected windowChunks(lineChunks: TextChunk[], startColumn: number, width: number): TextChunk[] {
        const out: TextChunk[] = [];
        let column = 0;
    ...
    update(palette: Palette): void {
        ...
        const left = Math.max(0, Math.min(this.anchorX + this.anchorOffsetColumns, Math.max(0, renderer.width - boxWidth)));
