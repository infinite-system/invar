...
class $OverlayLayer {
    ...
    constructor(protected readonly deps: OverlayLayerDeps) {
        ...
        this.quickOpenList.onMouseDown = (event) => {
            const row = quickOpenRowAt(event.y);
            if (row < 0 || row >= this.quickOpenRowCount)
                return;
        ...
        this.findBarText.onMouseDown = (event) => {
            ...
            const button = this.findBarButtonZones.find((zone) => zone.row === localRow && localColumn >= zone.startColumn && localColumn < zone.endColumn);
            if (button)
                this.runFindButton(button.action);
        };
        ...
        this.settingsText.onMouseDown = (event) => {
            ...
            const zone = this.settingsWidgetZones.find((candidate) => candidate.row === localRow && localColumn >= candidate.startColumn && localColumn < candidate.endColumn);
            if (!zone)
                return;
            this.deps.settingsPanel.select(zone.index);
            if (zone.action === 'dec')
                this.deps.settingsPanel.adjust(-1);
            else if (zone.action === 'inc')
                this.deps.settingsPanel.adjust(1);
    ...
    update(palette: Palette): void {
        ...
        if (settingsPanel.open.value) {
            ...
            for (const row of settingsRows) {
                ...
                const emit = (text: string, color: string, action?: SettingsWidgetZone['action']): void => {
                    ...
                    if (action)
                        zones.push({ row: textRow, startColumn: column, endColumn: column + text.length, index: row.index, action });
                    column += text.length;
                };
        ...
        if (shortcutHelp.open.value) {
            ...
            const sheetScrollHint = sheetRows.length > sheetViewportRows
        ...
        if (menuOpen) {
            ...
            contextMenu.items.value.forEach((item, index) => {
                ...
                const rowBackground = index === contextMenu.selectedIndex.value
                    ? palette.selection
                ...
                if (index < contextMenu.items.value.length - 1)
