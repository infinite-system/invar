...
class $StatusBar {
    ...
    constructor(protected readonly deps: StatusBarDeps) {
        ...
        deps.app.onDispose(() => { if (this.clockTimer)
        ...
        this.rightDockButton.onMouseMove = (event) => {
            ...
            const openChordHint = deps.keybindings.bindingHint('view.toggleRightDock', 'global');
            deps.tooltip.point(`Right dock${openChordHint ? ` (${openChordHint})` : ''}`, event.x, event.y);
        };
        ...
        this.shortcutHelpButton.onMouseMove = (event) => {
            ...
            deps.tooltip.point(`Keyboard shortcuts${openChordHint ? ` (${openChordHint})` : ''}`, event.x, event.y);
        };
    ...
    protected toggleSettings(): void {
        ...
        if (settingsPanel.open.value)
            ...
            overlayCoordinator.openExclusiveOverlay('settingsPanel', () => settingsPanel.toggle());
    }
    ...
    protected renderableContainsPoint(renderable: TextRenderable, column: number, row: number): boolean {
    ...
    protected renderStatus(markdownPreviewFocused: boolean): string {
        ...
        const focusLabel = workspaceSet.active.focus.value === 'files'
            ? '[Files]'
        ...
        if (app.copyNotice.value)
            parts.push(app.copyNotice.value);
        ...
        if (languageSizeNotice)
            parts.push(languageSizeNotice);
    ...
    update(palette: Palette, markdownPreviewFocused: boolean): void {
        ...
        const visibleContentIdentifiers = new Set(this.deps.panelHost.visibleContents().map((content) => content.id));
