...
class $GitPaneRenderer {
    protected static changeRowText(row: ChangeRow, checkboxIcons: CheckboxIconSet): string {
        if (row.kind === 'header')
    ...
    protected static commitLogRowText(row: CommitLogRow): string {
        if (row.kind === 'commit') {
    ...
    public static changesContentWidth(rows: readonly ChangeRow[], checkboxIcons: CheckboxIconSet): number {
        return rows.reduce((widestWidth, row) => Math.max(widestWidth, EditorCoordinates.Class.lineWidth(this.changeRowText(row, checkboxIcons))), 0);
    }
    public static logContentWidth(workspace: Workspace.Instance): number {
        let widestWidth = 0;
        for (const record of workspace.commitLog.value?.cache.value.values() ?? []) {
            widestWidth = Math.max(widestWidth, EditorCoordinates.Class.lineWidth(` ▸ ${record.shortSha} ${record.subject}`));
        }
        for (const expansion of workspace.commitExpansion.value?.entries.value ?? []) {
            for (const file of expansion.files ?? []) {
                widestWidth = Math.max(widestWidth, EditorCoordinates.Class.lineWidth(`    ${file.status} ${file.path}`));
            }
        }
        return widestWidth;
    }
    public static render(context: GitPaneRenderContext): {
        text: StyledText;
        geometry: GitPanelGeometry;
    } {
        ...
        const pushRow = (text: string, color: string, options: {
            ...
            if (options.background)
                chunk = bg(options.background)(chunk);
            chunks.push(chunk);
        ...
        if (!git)
            return { text: new StyledText([fg(palette.dim)('  no repository')]), geometry: emptyGeometry };
        ...
        const changesTop = Math.min(gitPanel.changesScrollTop.value, Math.max(0, changeRows.length - changesVisible));
        const glyphColor = (glyph: string): string => glyph === 'A' ? palette.added : glyph === 'D' ? palette.deleted : glyph === '?' ? palette.dim : palette.modified;
        changeRows.slice(changesTop, changesTop + changesVisible).forEach((row, visibleIndex) => {
            const rowIndex = changesTop + visibleIndex;
            if (row.kind === 'header') {
                pushRow(this.changeRowText(row, checkboxIcons), palette.dim, {
                    bold: true,
                ...
                const label = this.changeRowText(row, checkboxIcons);
                if ((selected && selectionFocused) || hovered) {
                    ...
                    const pathText = EditorCoordinates.Class.padToDisplayWidth(EditorCoordinates.Class.displayColumnWindow(label, gitPanel.changesScrollLeft.value, changesActiveNameWidth), changesActiveNameWidth);
                    const paint = (text: string, color: string) => background ? bg(background)(fg(color)(text)) : fg(color)(text);
        ...
        for (let filler = changesRendered; filler < changesVisible; filler++)
            pushRow('', palette.fg);
        ...
        if (commitLog) {
            ...
            const flatRows = GitLogRows.Class.commitLogRows(flatTop, logHeight, expandedEntries, (commitIndex) => windowRecords[commitIndex - firstCommitIndex], commitLog.knownEnd.value);
            flatRows.forEach((row, index) => {
                ...
                if (row.kind === 'commit') {
                    pushRow(this.commitLogRowText(row), row.record ? palette.fg : palette.dim, {
                        background,
                        newline,
                    ...
                    pushRow(this.commitLogRowText(row), palette.dim, {
                        background,
                        newline,
                        scrollLeft: gitPanel.logScrollLeft.value,
                        viewportWidth: logViewportWidth,
                    ...
                    pushRow(this.commitLogRowText(row), glyphColor(row.glyph), {
                        background,
                        newline,
                        scrollLeft: gitPanel.logScrollLeft.value,
                        viewportWidth: logViewportWidth,
