...
class $TabBarRenderer {
    ...
    protected static appendHorizontalGap(chunks: TextChunk[], color: string, gapWidth: number): void {
        if (gapWidth > 0)
            chunks.push(fg(color)(' '.repeat(gapWidth)));
    }
    public static renderWorkspace(context: WorkspaceTabBarRenderContext): WorkspaceTabBarRender {
        ...
        if (orientation === 'vertical') {
            ...
            if (activeWorkspaceIndex >= 0 && activeWorkspaceIndex !== revealedIndex) {
                if (activeWorkspaceIndex < strip.scrollOffset.value ||
            ...
            const endWorkspaceIndex = Math.min(workspaceTabs.length, startWorkspaceIndex + visibleWorkspaceCount);
            let rowIndex = 0;
            ...
            controlRows.forEach((control, controlIndex) => {
                ...
                const enabled = control.kind === 'add' ||
        ...
        const measuredWorkspaceTabs = workspaceTabs.map((workspaceTab) => ({
            workspaceTab,
            width: Math.min(availableTabsWidth, Math.min(this.workspaceTabMaxLabelWidth, Math.max(EditorCoordinates.Class.lineWidth(workspaceTab.label), EditorCoordinates.Class.lineWidth(workspaceTab.detailLabel ?? ''))) + 6),
        }));
        ...
        const visibleEndFrom = (startIndex: number): number => {
            ...
            for (let workspaceIndex = startIndex; workspaceIndex < measuredWorkspaceTabs.length; workspaceIndex += 1) {
                ...
                if (usedWidth + measuredWorkspaceTab.width > availableTabsWidth)
                    break;
        ...
        for (let workspaceIndex = startWorkspaceIndex; workspaceIndex < endWorkspaceIndex; workspaceIndex += 1) {
            ...
            const detailLabel = this.ellipsize(workspaceTab.detailLabel ?? '', maximumDetailWidth);
    ...
    public static renderBuffer(context: BufferTabBarRenderContext): BufferTabBarRender {
        const { strip, palette, hover } = context;
        const tabs = strip.items;
        const segments: TabBarSegment[] = [];
        let revealedIndex = context.lastRevealedIndex;
        if (tabs.length === 0)
            return { text: new StyledText([fg(palette.dim)('  no open files')]), segments, revealedIndex };
        ...
        const windowEndFrom = (start: number): number => {
            ...
            for (let index = start; index < total; index += 1) {
                ...
                if (!entry || used + entry.width > tabsAreaWidth)
                    break;
                used += entry.width;
                end = index + 1;
            }
        ...
        for (let index = startIndex; index < measured.length; index += 1) {
            ...
            const leadWidth = 1;
            if (!entry || column + leadWidth + entry.width > tabsAreaWidth)
                break;
            chunks.push(fg(palette.fg)(' '));
            column += leadWidth;
            ...
            const paint = (text: string, color: string) => rowBackground ? bg(rowBackground)(fg(color)(text)) : fg(color)(text);
            const start = column;
    ...
    public static breadcrumbNavButtonAt(localColumn: number): 'back' | 'forward' | null {
        if (localColumn === this.breadcrumbBackColumn || localColumn === this.breadcrumbBackColumn - 1)
            return 'back';
        if (localColumn === this.breadcrumbForwardColumn || localColumn === this.breadcrumbForwardColumn - 1)
            return 'forward';
        return null;
    }
    public static renderBreadcrumb(context: BreadcrumbBarRenderContext): StyledText {
        const { strip, palette, projectRoot, canGoBack, canGoForward } = context;
        const activeTab = strip.items.find((tab) => tab.active);
        if (!activeTab)
            return new StyledText([fg(palette.dim)('')]);
        const barWidth = Math.max(1, context.barWidth);
        const crumbs = Breadcrumb.Class.fitBreadcrumb(Breadcrumb.Class.breadcrumbSegments(activeTab.identifier, projectRoot), Math.max(1, barWidth - this.breadcrumbNavPrefixWidth - 1), // reserve the nav-button prefix + a trailing pad
        3);
...
export type TabBarHover = {
    kind: 'tab' | 'close' | 'previewToggle' | 'arrowLeft' | 'arrowRight' | 'badge';
    index: number;
} | null;
