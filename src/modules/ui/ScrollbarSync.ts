...
class $ScrollbarSync {
  ...
  protected readonly changesVerticalBar: ScrollBarRenderable;
  protected readonly changesHorizontalBar: ScrollBarRenderable;
  protected readonly logVerticalBar: ScrollBarRenderable;
  protected readonly logHorizontalBar: ScrollBarRenderable;
  constructor(protected readonly deps: ScrollbarSyncDeps) {
    ...
    const makeBar = (
      ...
      new SolidThumbScrollBar.Class(renderer, {
        ...
        ...(orientation === 'vertical'
          ? { width: deps.scrollbarThicknessCells() }
          : { height: deps.scrollbarThicknessCells() }),
        showArrows: false,
    ...
    this.editorVerticalBar = makeBar(
      ...
      (position) => {
        ...
        workspaceSet.active.editor.viewport.scrollTop.value =
          this.trueScrollPosition(this.editorVerticalBar, position);
      },
    );
    this.editorHorizontalBar = makeBar(
      ...
      (position) => {
        ...
        workspaceSet.active.editor.viewport.scrollLeft.value =
          this.trueScrollPosition(this.editorHorizontalBar, position);
      },
      {
        backgroundColor: readPalette().bg,
        foregroundColor: readPalette().accent,
      },
    );
    ...
    this.changesVerticalBar = makeBar(
      'git-changes-scrollbar-v',
      'vertical',
      (position) => {
        workspaceSet.active.haltGitChangesScroll();
        workspaceSet.active.gitPanel.changesScrollTop.value =
          this.trueScrollPosition(this.changesVerticalBar, position);
      },
    );
    this.changesHorizontalBar = makeBar(
      'git-changes-scrollbar-h',
      'horizontal',
      (position) => {
        workspaceSet.active.haltGitChangesHorizontalScroll();
        workspaceSet.active.gitPanel.changesScrollLeft.value =
          this.trueScrollPosition(this.changesHorizontalBar, position);
      },
      {
    ...
    this.logVerticalBar = makeBar(
      'git-log-scrollbar-v',
      'vertical',
      (position) => {
        workspaceSet.active.haltGitLogScroll();
        workspaceSet.active.gitPanel.logScrollTop.value =
          this.trueScrollPosition(this.logVerticalBar, position);
        workspaceSet.active.ensureLogWindow(
          workspaceSet.active.gitPanel.logScrollTop.value,
        );
      },
    );
    this.logHorizontalBar = makeBar(
      'git-log-scrollbar-h',
      'horizontal',
      (position) => {
        workspaceSet.active.haltGitLogHorizontalScroll();
        workspaceSet.active.gitPanel.logScrollLeft.value =
          this.trueScrollPosition(this.logHorizontalBar, position);
      },
      {
        backgroundColor: readPalette().panel,
        foregroundColor: readPalette().accent,
    ...
    this.treeVerticalBar = makeBar(
      ...
      (position) => {
        ...
        workspaceSet.active.tree.scrollTop.value = this.trueScrollPosition(
    ...
    this.treeHorizontalBar = makeBar(
      ...
      (position) => {
        ...
        workspaceSet.active.tree.scrollLeft.value = this.trueScrollPosition(
  ...
  protected applyBarGeometry(
    ...
    if (process.env.TUI_DEBUG_BARS === '1')
      Logging.Class.info(
        `bar ${bar.id}: scrollSize=${scroll.scrollSize} viewportSize=${scroll.viewportSize} scrollPosition=${scroll.scrollPosition} thickness=${thickness} trackLeft=${geometry.trackLeft} -> left=${bar.left} top=${bar.top} laidX=${bar.x} laidY=${bar.y} laidW=${bar.width} laidH=${bar.height} sliderViewPort=${bar.slider.viewPortSize} sliderMax=${bar.slider.max} sliderValue=${bar.slider.value} sliderH=${bar.slider.height}`,
      );
  }
  ...
  syncPaneViewportGeometry(): boolean {
    ...
    if (filesVisible) {
      const treeViewportHeight = Math.max(
        1,
        (this.deps.sidebar.height as number) - 2,
      );
      const treeViewportWidth = Math.max(
        1,
        sidebarInnerWidth - this.deps.scrollbarThicknessCells(),
      );
      if (
    ...
    const changesContentWidth = gitAvailable
      ? GitPaneRenderer.Class.changesContentWidth(
          this.deps.gitChangeRowsNow() as never,
          theme.checkboxIcons,
        )
      : 0;
    ...
    if (
      workspaceSet.active.gitPanel.changesViewportHeight.value !==
    ...
    if (
      workspaceSet.active.gitPanel.changesViewportWidth.value !==
        changesViewportWidth ||
      workspaceSet.active.gitPanel.changesContentWidth.value !==
        changesContentWidth
    ) {
      workspaceSet.active.gitPanel.setChangesHorizontalExtent(
        changesContentWidth,
        changesViewportWidth,
      );
      changed = true;
    }
    const logViewportWidth = Math.max(
      1,
      sidebarInnerWidth - this.deps.scrollbarThicknessCells(),
    );
    const logContentWidth = gitAvailable
      ? GitPaneRenderer.Class.logContentWidth(workspaceSet.active)
      : 0;
    if (
      workspaceSet.active.gitPanel.logViewportWidth.value !==
        logViewportWidth ||
      workspaceSet.active.gitPanel.logContentWidth.value !== logContentWidth
    ) {
      workspaceSet.active.gitPanel.setLogHorizontalExtent(
        logContentWidth,
        logViewportWidth,
      );
      changed = true;
  ...
  syncScrollbars(): void {
    const { workspaceSet, editorArea, codeBody, sidebar, gitSplitDivider } =
      this.deps;
    ...
    this.applyBarGeometry(
      ...
      {
        scrollSize:
          editorVisible && !editor.wordWrap.value
    ...
    const gitVisible =
      workspaceSet.active.sidebarView.value === 'git' &&
      workspaceSet.active.git.value !== null;
    ...
    const changesRegion = {
      ...
      height: Math.max(1, geometry.changesRows),
    };
    ...
    const changesViewportWidth =
      workspaceSet.active.gitPanel.changesViewportWidth.value;
    this.applyBarGeometry(
      this.changesHorizontalBar,
      'horizontal',
      changesRegion,
      {
    ...
    const logRegion = {
      ...
      top:
        geometry.logHeaderRow >= 0
          ? geometry.dividerRow + 1
          : geometry.dividerRow,
      left: 0,
      width: sidebarInnerWidth,
    ...
    this.applyBarGeometry(this.logHorizontalBar, 'horizontal', logRegion, {
      scrollSize: gitVisible
        ? workspaceSet.active.gitPanel.logContentWidth.value
        : 0,
