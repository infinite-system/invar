...
class $ScrollbarSync {
    ...
    constructor(protected readonly deps: ScrollbarSyncDeps) {
        ...
        const makeBar = (id: string, orientation: 'vertical' | 'horizontal', onChange: (position: number) => void, trackOptions?: {
            backgroundColor: ColorInput;
            foregroundColor: ColorInput;
        }): ScrollBarRenderable => new SolidThumbScrollBar.Class(renderer, {
            id,
            orientation,
            position: 'absolute',
            ...
            onChange: (position) => {
                if (this.applying)
                    return; // ignore our own per-frame scrollPosition sync (One-Writer)
                onChange(position);
            },
        });
    ...
    protected applyBarGeometry(bar: ScrollBarRenderable, orientation: 'vertical' | 'horizontal', region: {
        top: number;
        left: number;
        width: number;
        height: number;
        ...
        const slider = (bar as unknown as {
            slider?: {
                width?: number;
                height?: number;
            };
        ...
        if (process.env.TUI_DEBUG_BARS === '1')
            Logging.Class.info(`bar ${bar.id}: thickness=${thickness} trackLeft=${geometry.trackLeft} -> left=${bar.left} top=${bar.top} laidX=${bar.x} laidY=${bar.y} laidW=${bar.width} laidH=${bar.height} sliderViewPort=${bar.slider.viewPortSize} sliderMax=${bar.slider.max} sliderValue=${bar.slider.value} sliderH=${bar.slider.height}`);
    }
    ...
    syncPaneViewportGeometry(): boolean {
        ...
        if (workspaceSet.active.gitPanel.changesViewportHeight.value !== changesViewportHeight ||
        ...
        if (workspaceSet.active.gitPanel.changesViewportWidth.value !== changesViewportWidth ||
        ...
        if (workspaceSet.active.gitPanel.logViewportWidth.value !== logViewportWidth ||
    ...
    syncScrollbars(): void {
        ...
        this.applyBarGeometry(this.treeVerticalBar, 'vertical', { top: 0, left: 0, width: sidebarInnerWidthFiles, height: treeViewportHeight }, {
        ...
        this.applyBarGeometry(this.treeHorizontalBar, 'horizontal', { top: 0, left: 0, width: sidebarInnerWidthFiles, height: treeViewportHeight }, {
