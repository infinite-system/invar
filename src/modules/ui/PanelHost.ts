...
class $PanelHost {
    ...
    register(content: PaneContent): void {
        ...
        if (this.activeId.value === null)
            this.activeId.value = content.id;
        ...
        if (this.options.showWhenContentRegistered)
    ...
    get resolvedCells(): ResolvedPanelCell[] {
        ...
        for (const cell of layout) {
            ...
            if (content)
                resolved.push({ content, ratio: Math.max(0, cell.ratio) });
        }
    ...
    activate(id: string): void {
        if (!this.contents.has(id) || this.activeId.value === id)
            return;
        this.retargetFocus(() => {
    ...
    split(ids: string[], ratios?: number[]): void {
        ...
        this.retargetFocus(() => {
            ...
            if (this.focusedIndex.value >= valid.length)
                this.focusedIndex.value = 0;
    ...
    focusCell(index: number): void {
        ...
        if (clamped === this.focusedIndex.value
    ...
    toggleContent(id: string): void {
        ...
        const visibleIndex = visibleCells.findIndex((cell) => cell.content.id === id);
        if (visibleIndex < 0) {
            const visibleIdentifiers = new Set(visibleCells.map((cell) => cell.content.id));
            visibleIdentifiers.add(id);
            const orderedIdentifiers = this.order.value.filter((identifier) => visibleIdentifiers.has(identifier));
            this.retargetFocus(() => {
        ...
        const remainingCells = visibleCells.filter((cell) => cell.content.id !== id);
        const remainingTotal = remainingCells.reduce((sum, cell) => sum + cell.ratio, 0) || 1;
        const preferredIdentifier = focusedIdentifier !== id
            ? focusedIdentifier
            : remainingCells[Math.min(visibleIndex, remainingCells.length - 1)]?.content.id ?? remainingCells[0]?.content.id ?? null;
        this.retargetFocus(() => {
            ...
            this.focusedIndex.value = Math.max(0, remainingCells.findIndex((cell) => cell.content.id === preferredIdentifier));
        });
    }
    ...
    moveDivider(dividerIndex: number, boundaryFraction: number): void {
        ...
        if (dividerIndex < 0 || dividerIndex >= cells.length - 1)
            return;
    ...
    cellSpans(totalColumns: number): PanelCellSpan[] {
        ...
        return cells.map((cell, index) => {
            ...
            const columns = index === cells.length - 1
