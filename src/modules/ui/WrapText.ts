...
class $WrapText {
        ...
        logicalLines.forEach((logicalLine, logicalIndex) => {
            ...
            if (clusters.length === 0) {
                ...
                for (const cluster of clusters) {
                    ...
                    if (rowCount > 0 && rowWidth + clusterWidth > budget)
                        flush();
                    rowText += cluster;
                    rowCount += 1;
    ...
    public static visualPositionOf(segments: readonly WrapSegment[], graphemeIndex: number): VisualPosition {
        ...
        for (let index = 0; index < segments.length; index += 1) {
            ...
            if (graphemeIndex < endOffset || (graphemeIndex === endOffset && (!nextContinuesLine || isLast))) {
                ...
                return { line: index, column: this.displayWidth(EditorCoordinates.Class.graphemes(segment.text).slice(0, within).join('')) };
            }
        }
        const last = segments[segments.length - 1]!;
        return { line: segments.length - 1, column: last.displayWidth };
    }
    public static graphemeAtVisualPosition(segments: readonly WrapSegment[], line: number, column: number): number {
        if (segments.length === 0)
            return 0;
    ...
    public static sliceByDisplayCells(text: string, startCell: number, endCell: number): string {
    ...
    public static clipToWidth(text: string, cells: number, ellipsis = '…'): string {
        if (cells <= 0)
            return '';
        if (this.displayWidth(text) <= cells)
            return text;
        const ellipsisWidth = this.displayWidth(ellipsis);
        return this.sliceByDisplayCells(text, 0, Math.max(0, cells - ellipsisWidth)) + ellipsis;
    }
}
