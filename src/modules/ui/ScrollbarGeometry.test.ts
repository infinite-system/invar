...
const shapes: Array<{ name: string; region: RegionRect }> = [
  ...
  {
    name: 'log region low split',
    region: { top: 20, left: 0, width: 30, height: 17 },
  },
  {
    name: 'log region high split',
    region: { top: 7, left: 0, width: 30, height: 30 },
  },
  ...
  for (const { name, region } of shapes) {
    test(`vertical track hugs the right edge within the region (${name})`, () => {
      const geometry = ScrollbarGeometry.Class.scrollbarGeometry(
        'vertical',
        region,
        { scrollSize: 1000, viewportSize: region.height, scrollPosition: 0 },
      );
    ...
    test(`horizontal track hugs the bottom edge within the region (${name})`, () => {
      const geometry = ScrollbarGeometry.Class.scrollbarGeometry(
        'horizontal',
        region,
        { scrollSize: 500, viewportSize: region.width, scrollPosition: 0 },
      );
      expect(geometry).not.toBeNull();
...
describe('hidden when content fits', () => {
  test('returns null when scrollSize <= viewportSize', () => {
    expect(
      ScrollbarGeometry.Class.scrollbarGeometry('vertical', shapes[0]!.region, {
        scrollSize: 10,
        viewportSize: 37,
        scrollPosition: 0,
      }),
...
describe('exact extremes round-trip through the reported scale', () => {
  for (const { name, region } of shapes) {
    test(`position 0 and max map to reported 0 and reported max (${name})`, () => {
      ...
      const atStart = ScrollbarGeometry.Class.scrollbarGeometry(
        'vertical',
        region,
        { scrollSize, viewportSize, scrollPosition: 0 },
      )!;
      const atEnd = ScrollbarGeometry.Class.scrollbarGeometry(
        'vertical',
        region,
        { scrollSize, viewportSize, scrollPosition: trueMax },
      )!;
      expect(atStart.reportedPosition).toBe(0);
      expect(atEnd.reportedPosition).toBe(
        scrollSize - atEnd.reportedViewportSize,
      ); // reported max
      ...
      if (reportedRange > 0) {
        ...
        expect(
          Math.round(atEnd.reportedPosition * atEnd.reportedToTrueScale),
        ).toBe(trueMax);
      } else {
...
describe('minimum thumb', () => {
  test('the reported ratio never yields a thumb below the minimum, even on huge content', () => {
    ...
    const geometry = ScrollbarGeometry.Class.scrollbarGeometry(
      'vertical',
      region,
      { scrollSize: 100000, viewportSize: 17, scrollPosition: 0 },
    )!;
    const thumbCells = Math.max(
      1,
      Math.round(
        (geometry.reportedViewportSize / 100000) * geometry.trackLength,
      ),
    );
