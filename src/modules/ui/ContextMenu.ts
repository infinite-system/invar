...
class $ContextMenu {
    ...
    protected runHandler: ((itemId: string) => void) | null = null;
    /** Rendered box width in cells: the widest label plus padding and borders. */
    get width(): number {
        let widestLabel = 0;
        for (const item of this.items.value)
            widestLabel = Math.max(widestLabel, item.label.length);
        return widestLabel + this.menuHorizontalFrame;
    }
    /** Rendered box height in cells: one row per item plus the borders. */
    get height(): number {
        return this.items.value.length + this.menuVerticalFrame;
    }
    /**
     ...
     */
    openAt(items: readonly ContextMenuItem[], pointerX: number, pointerY: number, screen: {
        width: number;
        height: number;
    }, onRun: (itemId: string) => void): void {
        if (items.length === 0)
            return;
    ...
    moveSelection(direction: 1 | -1): void {
        ...
        for (let index = this.selectedIndex.value + direction; index >= 0 && index < items.length; index += direction) {
