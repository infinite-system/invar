...
class $EditorPane {
    ...
    renderEditor(): {
        ...
        const bracketMatch = editor.hasDocument.value && !workspaceSet.active.showingDiff.value
            ? BracketMatch.Class.findInDocument(editor.document, editor.cursor.line.value, editor.cursor.col.value, LanguageRegistry.Class.forPath(editor.document.path))
            : null;
    ...
    wrapVisualPosition(line: number, column: number): {
        rowIndex: number;
        column: number;
    } | 'before' | 'after' {
        ...
        if (line > lastRow.lineIndex || (line === lastRow.lineIndex && segmentIndex > lastRow.segmentIndex))
            return 'after';
        const rowIndex = this.wrapRowsWindow.findIndex((row) => row.lineIndex === line && row.segmentIndex === segmentIndex);
        if (rowIndex < 0)
            return 'after';
    ...
    applySelection(): void {
        ...
        if (editor.wordWrap.value) {
            ...
            const focusCell = endPosition === 'after'
                ? { rowIndex: this.wrapRowsWindow.length - 1, column: editorViewportWidth() }
                : endPosition;
            codeBody.setSelectionRange(Math.max(0, anchorCell.column), anchorCell.rowIndex, Math.max(0, focusCell.column), focusCell.rowIndex);
            return;
        }
        ...
        const focusX = selection.end.line < top + viewportHeight
        ...
        codeBody.setSelectionRange(Math.max(0, anchorX - selectionScrollLeft), anchorY, Math.max(0, focusX - selectionScrollLeft), focusY);
    }
    // Public so RootView/HoverCard can map a screen cell to a document position (mirrors wrapVisualPosition).
    documentPositionAtCell(cellX: number, cellY: number): {
        line: number;
        column: number;
    } | null {
        ...
        if (workspaceSet.active.editor.wordWrap.value) {
            ...
            if (this.wrapRowsWindow.length === 0)
                return null;
            ...
            const hitColumn = EditorCoordinates.Class.graphemeAtDisplayColumn(lineText, row.segment.startDisplayColumn + Math.max(0, cellX - codeBody.x));
        ...
        const line = Math.max(0, Math.min(workspaceSet.active.editor.viewport.scrollTop.value + (cellY - codeBody.y), workspaceSet.active.editor.document.lineCount - 1));
        const column = EditorCoordinates.Class.graphemeAtDisplayColumn(workspaceSet.active.editor.document.line(line), workspaceSet.active.editor.viewport.scrollLeft.value + (cellX - codeBody.x));
        return { line, column };
    }
    ...
    protected pointerIsOverText(cellX: number, cellY: number, position: {
        line: number;
        column: number;
    }): boolean {
        ...
        if (!editor.wordWrap.value) {
            ...
            if (displayColumn < 0 || displayColumn >= EditorCoordinates.Class.lineWidth(lineText))
                return false;
        }
    ...
    protected buildDragBehavior(): SelectionDragBehavior.Model {
        ...
        return new SelectionDragBehavior.Class({
            ...
            lineGraphemeCount: (lineIndex) => EditorCoordinates.Class.graphemeCount(workspaceSet.active.editor.document.line(lineIndex)),
            ...
            scrollColumns: (columnDelta) => {
                workspaceSet.active.editor.viewport.scrollByColumns(columnDelta, workspaceSet.active.editor.document.maximumLineWidth);
            },
    ...
    protected wireHandlers(): void {
        ...
        codeBody.onMouseDown = (event) => {
            ...
            if (clickPosition) {
                ...
                const sameSpot = clickPosition.line === this.lastClickLine &&
