 ...
 */
class $TabStrip {
    constructor(initialOrientation: TabStripOrientation = 'horizontal', protected readonly readItems: () => readonly TabStripItem[] = () => []) {
    ...
    pan(itemDelta: number): void {
        this.scrollOffset.value = Math.max(0, Math.min(this.scrollOffset.value + itemDelta, Math.max(0, this.items.length - 1)));
    }
    clampScrollOffset(maximumScrollOffset: number): void {
        this.scrollOffset.value = Math.max(0, Math.min(this.scrollOffset.value, Math.max(0, maximumScrollOffset)));
    }
}
