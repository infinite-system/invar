...
import { BoxRenderable, TextRenderable, StyledText, fg, bg, bold, type TextChunk, type CliRenderer, type MouseEvent, } from '@opentui/core';
...
class $RootView {
    public static buildRootView(renderer: CliRenderer, workspaceSet: WorkspaceSet.Instance, bufferTabStrip: TabStrip.Instance, workspaceTabStrip: TabStrip.Instance, theme: Theme.Instance, keybindings: KeybindingRegistry.Instance, commands: CommandRegistry.Instance, app: App.Instance, contextMenu: ContextMenu.Instance, boundedListPopup: BoundedListPopup.Instance, tooltip: Tooltip.Instance, settingsPanel: SettingsPanel.Instance, findBar: FindBar.Instance, quickOpen: QuickOpen.Instance, shortcutHelp: ShortcutHelp.Instance, overlayCoordinator: OverlayCoordinator.Instance, panelHost: PanelHost.Instance, primaryDockHost: PanelHost.Instance, rightDockHost: PanelHost.Instance, toggleTerminal: () => void, toggleAgent: () => void, toggleRightDock: () => void, activateQuickOpen: () => void, revealFindMatch: () => void): RootView {
        ...
        const breadcrumbButtonsShown = (): boolean => workspaceSet.active.editor.hasDocument.value && !workspaceSet.active.showingDiff.value;
        breadcrumbBar.onMouseDown = (event) => {
            if (!breadcrumbButtonsShown())
                return;
        ...
        rightDockBody.onMouseDown = (event: MouseEvent) => {
            panelHost.blur();
            rightDockHost.focus();
            rightDockHost.activeContent?.onPointerDown?.(Number(event.x) - Number(rightDockBody.x), Number(event.y) - Number(rightDockBody.y));
            renderer.requestRender();
        };
        rightDockBody.onMouseScroll = (event) => {
            ...
            if (direction !== 'up' && direction !== 'down')
                return;
            rightDockHost.activeContent?.onWheel?.((direction === 'up' ? -1 : 1) * wheelStep(event));
            renderer.requestRender();
        };
        ...
        const initialLayoutRows = Math.max(1, renderer.height
            - 1
            - 1
            - (settings.workspaceTabPosition.value === 'top' ? 2 : 0));
        let panelHeightRows = LayoutModel.Class.defaultBottomPanelRows(initialLayoutRows);
        ...
        const panelBoundaryFraction = (dividerIndex: number): number => {
            ...
            let fraction = 0;
            for (let index = 0; index <= dividerIndex && index < cells.length; index += 1)
        ...
        const ensurePanelCellView = (index: number): PanelCellView => {
            ...
            body.onMouseDrag = (event: MouseEvent) => {
                ...
                if (!agent) {
                    const content = panelHost.resolvedCells[index]?.content;
                    content?.onPointerDrag?.((event.x as number) - (body.x as number), (event.y as number) - (body.y as number));
                    renderer.requestRender();
                    return;
                }
                if (composerDragging) {
            ...
            const endAgentDrag = (event: MouseEvent): void => {
                ...
                if (!agent) {
                    const content = panelHost.resolvedCells[index]?.content;
                    content?.onPointerUp?.((event.x as number) - (body.x as number), (event.y as number) - (body.y as number));
                    renderer.requestRender();
                    return;
                }
                if (composerDragging) {
        ...
        const syncPanelCellMount = (count: number): void => {
            if (count === mountedPanelCellCount)
                return;
            ...
            for (let index = 0; index < count; index += 1) {
                ...
                if (view.splitterElement)
                    panelBox.add(view.splitterElement.renderable);
        ...
        const panelViewportColumns = (): number => panelHost.visible.value
            ? Math.max(1, layoutSlotGeometry.bottomPanel.width - 2)
            : 0;
        const panelViewportRows = (): number => panelHost.visible.value
        ...
        const panelContainsPoint = (x: number, y: number): boolean => {
            ...
            return (statusBar.panelControlContainsPoint(x, y)
                || (x >= boxX
                    && x < boxX + boxWidth
                    && y >= boxY - 1
                    && y < boxY + boxHeight));
        };
        const rightDockViewportColumns = (): number => rightDockHost.visible.value
            ? Math.max(1, layoutSlotGeometry.rightDock.width - 2)
            : 0;
        const rightDockViewportRows = (): number => rightDockHost.visible.value
        ...
        const synchronizeLayoutGeometry = (): void => {
            const fallbackColumns = Math.max(1, renderer.width -
                (settings.workspaceTabPosition.value === 'left' ? 22 : 0));
            const fallbackRows = Math.max(1, renderer.height -
                1 -
                1 -
                (settings.workspaceTabPosition.value === 'top' ? 2 : 0));
            const totalColumns = Number(layoutCanvas.width) > 0
                ? Number(layoutCanvas.width)
                : fallbackColumns;
            const totalRows = Number(layoutCanvas.height) > 0
        ...
        const editorViewportWidth = () => {
            ...
            if (laidOut && laidOut > 1)
                return Math.max(1, laidOut - 1);
        ...
        function synchronizeWorkspaceTabMount(): void {
            ...
            if (position === workspaceTabBarMountedPosition)
                return;
            if (position === 'left') {
        ...
        function update(): void {
            ...
            if (rightDockHost.visible.value && rightDockHost.focused.value) {
                ...
                if (caret) {
                    renderer.setCursorPosition(rightDockBody.x + caret.column + 1, rightDockBody.y + caret.row + 1, true);
                }
                else {
            ...
            if (editor.wordWrap.value) {
                ...
                // invariant: The caret renders at the cursor display column (ui.invariants.md)
                const caretPosition = editor.hasDocument.value && !activeFileIsImage && workspaceSet.active.focus.value === 'editor' && !editorContentMount.markdownSplitView?.previewFocused && !commands.open.value
            ...
            const caretVisibleHorizontally = EditorCoordinates.Class.displayColumn(editor.document.line(Math.min(cursorLine, editor.document.lineCount - 1)), editor.cursor.col.value) >=
        ...
        const scrollModifierHeld = (event: WheelModifiers, modifier: ScrollModifier): boolean => ScrollGesture.Class.modifierHeld(event, modifier);
        ...
        return {
            ...
            dismissHoverSoft: () => { if (!hoverCard.engaged())
                hoverCard.clear(); },
...
export interface RootView {
    ...
    layoutGeometry(): LayoutSlotGeometry;
    splitterRegions(): Record<'sidebar' | 'git' | 'bottomPanel' | 'rightDock', {
        left: number;
        top: number;
        width: number;
        height: number;
