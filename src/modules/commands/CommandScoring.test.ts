...
test('fuzzyScore matches subsequences and rejects non-matches', () => {
  expect(
    CommandScoring.Class.fuzzyScore('sv', 'File: Save'),
  ).toBeGreaterThanOrEqual(0);
  expect(
    CommandScoring.Class.fuzzyScore('save', 'File: Save'),
  ).toBeGreaterThanOrEqual(0);
...
test('tighter (adjacent) matches score lower than spread-out ones', () => {
  ...
  const spread = CommandScoring.Class.fuzzyScore(
    'save',
    'Show a value everywhere',
  );
