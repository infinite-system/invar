...
- **`.claude/skills/ui-design/SKILL.md`** — the UI design doctrine: buttons, dialogs, flows, text inputs, scroll areas, and copy text. One geometry per control family, shared input model, cancel spine, drive-addressable flows. Load before building or changing ANY visible surface; new windows must compose from these chapters, never invent parallel widgets.
