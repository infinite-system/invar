...
Public classes use the namespace pattern ──────► Live static reads follow the receiving class
                                               ├─► Construction goes through overridable seams
                                               └─► The app is built only after the kernel is sealed
Live static reads follow the receiving class ──► Construction goes through overridable seams
...
- [Public classes use the namespace pattern][namespace] supplies the class receivers constrained by [Live static reads follow the receiving class][live-static], the live `Class`-slot component of [Construction goes through overridable seams][seam], and the selection slots finalized by [The app is built only after the kernel is sealed][seal]; [live static reads][live-static] supply the self-dispatch component of the construction seam, while the `createX()` factory component remains separate
...
**Members:** [Public classes use the namespace pattern][namespace] · [Live static reads follow the
receiving class][live-static] · [Imported dependencies are read late][late] · [Construction goes
through overridable seams][seam] · [The app is built only after the kernel is sealed][seal] — on
...
`Class` forms; receiver-following self-reads preserve subclass dispatch; late reads keep cycles
from breaking at load; the mutable `Class` slot lets the kernel compose; sealing-before-construct
means every instance sees the final class graph. Remove the namespace form and the class graph
loses its uniform selection seam; pin a self-read and an inherited call returns to its base; remove
late reads and load order matters; construct before seal and instances predate their plugins.
...
### Source text state survives replaceable projection

**Members:** [ivue owns state and OpenTUI owns projection][ownership] · [Data flows one way][flow] ·
[Document identity survives document instance replacement][document-identity] ·
[Renderables hold no model state][stateless-renderables] ·
[The source text editor is a pane content citizen][source-text-pane].
**Guarantee:** source-text model state remains authoritative while its view and renderables can be
disposed and rebuilt; persistent fold state survives the replacement.
**Mechanism of conjunction:** the stable `DocumentHandle` retains document-adjacent fold state and
attaches it to each replacement view. `SourceTextPaneContent` then pulls current model state through
the pane seam and paints presentation-only renderables without writing model state during paint.
**Breaks if:** fold state moves onto the disposable view; a renderable becomes cursor, selection, or
scroll truth; the host paints source text outside `PaneProjection`; paint writes back into a model.
...
[live-static]: project.invariants.md#live-static-reads-follow-the-receiving-class
[seal]: project.invariants.md#the-app-is-built-only-after-the-kernel-is-sealed
[flow]: project.invariants.md#data-flows-one-way
[ownership]: project.invariants.md#ivue-owns-state-and-opentui-owns-projection
...
[document-identity]: src/modules/workspace/workspace.invariants.md#document-identity-survives-document-instance-replacement
[stateless-renderables]: src/modules/ui/ui.invariants.md#renderables-hold-no-model-state
[source-text-pane]: src/modules/ui/ui.invariants.md#the-source-text-editor-is-a-pane-content-citizen
