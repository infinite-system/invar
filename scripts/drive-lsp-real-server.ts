...
function check(label: string, passed: boolean, detail: unknown): void {
  console.log(
    `${passed ? 'PASS' : 'FAIL'}  ${label}  ${JSON.stringify(detail)}`,
  );
  if (!passed) failures++;
}
...
try {
  ...
  const definitionLocation = await client.definition(barDocument, {
    line: useLineIndex,
    column: useColumn,
  });
  check(
  ...
  const hoverResult = await client.hover(barDocument, {
    line: useLineIndex,
    column: useColumn,
  });
  check(
    ...
    hoverResult !== null &&
      hoverResult.contents.includes('greetWidget(name: string): string'),
  ...
  while (
    client.diagnosticCountFor(barDocument) === 0 &&
    Date.now() - waitStart < 20_000
  ) {
  ...
  check(
    ...
    diagnostics.some(
      (diagnostic) =>
        diagnostic.code === 2322 && diagnostic.range.start.line === 3,
    ),
  ...
  const zwjStatement =
    "const zwjLabel = '👨‍👩‍👧'; export const zwjUse = greetWidget(zwjLabel);";
  ...
  const zwjResolved =
    ...
    ((zwjDefinition.uri.endsWith('/foo.ts') &&
      zwjDefinition.range.start.column === 16 &&
      zwjDefinition.range.end.column === 27) ||
      (zwjDefinition.uri.endsWith('/bar.ts') &&
        zwjDefinition.range.start.column === 9 &&
        zwjDefinition.range.end.column === 20));
  check(
    ...
    zwjResolved,
    {
      utf16: zwjUtf16,
      grapheme: zwjColumn,
      codePoints: Array.from(zwjLineText.slice(0, zwjUtf16)).length,
      location: zwjDefinition,
    },
  );
} finally {
  await client.dispose();
  check(
    'dispose reaches terminal disposed status',
    client.status.value === 'disposed',
    client.status.value,
  );
