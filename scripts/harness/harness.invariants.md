...
### Atomicity is claimed only for self-generated output

**Invariant:** If a harness asserts that a rendered surface is never blank, torn, or partial, then
that surface must be generated by Invar itself. For output produced by a foreign process, the
harness may assert only CONVERGENCE — that the observed frames do not end in the incomplete state —
never the absence of incomplete intermediate states.

**Scope:** Every `scripts/harness/smoke-*-harness.ts` assertion over `completedFrameObservations`
whose region contains output written by a child process: terminal panes running a shell or command,
`git` output surfaced verbatim, and any pane whose body bytes arrive from outside Invar. Invar's own
chrome — panels, tabs, breadcrumbs, gutters, status rows — is self-generated and keeps the strict
absence claim.

**Mechanism:** Invar paints its own surfaces inside synchronized-output markers, so a completed
frame containing Invar chrome is by construction complete. A foreign process has no such contract: a
program that repaints by clearing and redrawing emits a genuinely empty screen between the clear and
the redraw. A completed frame painted there reports the truth of what arrived. No terminal emulator
can coalesce a foreign repaint it cannot see the boundaries of, so an absence claim over foreign
output demands a guarantee the app is structurally unable to make, and its verdict then tracks
machine load instead of correctness.

**Generates:** The convergence shape used by the `tasks:watch` assertion in
`smoke-terminal-harness.ts` — a trailing-blank count that must be zero, with the longest transient
blank run reported and never gated. It also predicts which existing flake reports are overclaims
rather than defects: any report whose failure is an incomplete intermediate state in foreign output.

**Rejected alternatives:** Widening a frame budget or tolerance until the transient fits — converts
the defect into a slower version of itself and invents a threshold nobody can defend. Retrying the
assertion — the observation is total over committed frames, so a retry re-rolls the child's timing
rather than re-observing the same claim. Debouncing Invar's frame commit until foreign output
"settles" — no boundary exists to settle against, and it would delay real output to hide a
cosmetic transient.

**Evidence:** `scripts/harness/smoke-terminal-harness.ts`, the `real tasks:watch converges to
painted content` assertion and the PRESENT-arm assertion above it. Task #436 records the population:
the previous absence claim failed roughly 40% of full-pool gate runs on unchanged code while passing
solo. A quiet run of the converged assertion reports `longest transient blank run 1`, so the
transient occurs even unloaded — the old claim was false on a quiet machine too, and load only
raised the odds of catching it.

**Impossible if true:** No harness assertion may require that a foreign process's intermediate
repaint states are absent from committed frames. A blank frame followed by a painted frame can never
be a failure; a run of frames that ends blank can never be a pass.

**Verification:** `bun scripts/harness/smoke-terminal-harness.ts` reports both the PRESENT arm and
the convergence arm. Invert `tasksWatchFrameBearsContent` for the final observation only: the
convergence assertion must go red with a non-zero trailing-blank count.
...
without a visible change; frame-history assertions that cannot stop on a faster external side effect
before they observe the frame they judge.
...
`scripts/harness/smoke-terminal-stage-harness.ts` requires both its shell proof file and an observed
command frame before it judges reduced-motion first-frame completeness.
