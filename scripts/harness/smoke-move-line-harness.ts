...
function lineRows(snapshot: HarnessSnapshot.Model): number[] | null {
  ...
  const rows = lineMarkers.map((lineMarker) => snapshot.findText(lineMarker)?.row ?? -1);
  return rows.every((row) => row >= 0) ? rows : null;
}

function hasVisibleOrder(snapshot: HarnessSnapshot.Model, expectedLines: readonly string[]): boolean {
...
const homeDirectory = mkdtempSync(join(tmpdir(), 'tui-move-line-harness-home-'));
...
try {
  ...
  let snapshot = await driver.awaitSnapshot(
    (candidate) => hasVisibleOrder(candidate, ['one', 'two', 'three']),
  );
  driver.sendKeys('Tab');
  ...
  snapshot = await runCommand(
    driver,
    'Move Line Down',
    (candidate) => hasVisibleOrder(candidate, ['two', 'one', 'three']),
  );
  ...
  console.log('== harness move line: Duplicate Line inserts a copy and undo removes it ==');
  snapshot = await runCommand(
    driver,
    'Duplicate Line',
    (candidate) => {
      const oneRows = candidate.textRows()
        .map((text, row) => ({ text, row }))
        .filter(({ text }) => text.includes('one'))
        .map(({ row }) => row);
      return oneRows.length >= 2 && oneRows[1] === oneRows[0]! + 1;
    },
  );
  const visibleOneRows = snapshot.textRows()
    .map((text, row) => ({ text, row }))
    .filter(({ text }) => text.includes('one'))
    .map(({ row }) => row);
  requireCondition(
  ...
  driver.sendKeys('Control+z');
  await driver.awaitSnapshot((candidate) => hasVisibleOrder(candidate, ['one', 'two', 'three']));
