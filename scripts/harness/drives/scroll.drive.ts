...
const before = await app.get('workspaceSet.activeEditor.viewport.firstVisible');
await app.scroll('down', 8, editorColumn, editorRow);
await app.waitForRepaint();
const down = await app.get('workspaceSet.activeEditor.viewport.firstVisible');
console.log('scrolled:', before, '->', down);
await app.scroll('up', 10);
await app.waitForRepaint();
console.log(
  'returned to:',
  await app.get('workspaceSet.activeEditor.viewport.firstVisible'),
);
