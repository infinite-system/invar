# Brief #478 round 1 — migrate the pre-satisfied waits
...
1. [task-478](task-478-migrate-the-pre-satisfied-waits.md)
...
1. The FIVE pre-satisfied positive controls (census marks them) — each makes
   a claim unfalsifiable today. Both arms per conversion: plant the defect
   the control guards, prove the converted control goes red, restore, green.
2. The contention-tier files (plugin-manifest, panel-chrome leftovers,
   scrollbars) — these are the gate reds.
3. The remaining class-1 sites, file by file. Class-3 sleeps: convert only
   where a condition is obvious; do not chase all ~290.
