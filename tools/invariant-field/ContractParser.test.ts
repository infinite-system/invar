...
**Evidence:** \`tools/invariant-field/ContractParser.test.ts\`
**Impossible if true:** The same input produces two different outputs.
**Verification:** \`bun test tools/invariant-field/ContractParser.test.ts\`
**Status:** established
...
`;
  ...
  test('executes only bounded read-only verification on the current tree', () => {
    ...
    record.fields.Verification =
      '`grep -q "Invariance Field" tools/invariant-field/README.md`';
    expect(verificationMode(record, repositoryRoot, true)).toBe(
      'executed-pass',
    );
    record.fields.Verification =
      '`grep -q "not present in the guide" tools/invariant-field/README.md`';
    expect(verificationMode(record, repositoryRoot, true)).toBe(
      'executed-fail',
    );
    record.fields.Verification =
      '`bun test tools/invariant-field/ContractParser.test.ts`';
    expect(verificationMode(record, repositoryRoot, true)).toBe(
      'citation-only',
    );
